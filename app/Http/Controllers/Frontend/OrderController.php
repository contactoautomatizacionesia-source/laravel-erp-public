...
use Modules\PaymentGateway\Http\Controllers\EpaycoController;
...
class OrderController extends Controller
{
    ...
    protected $orderService, $ordermanageService;
    public function __construct(OrderService $orderService, OrderManageService $orderManageService)
    {
    ...
    public function store(Request $request)
    {
        ...
        \Log::info('store OrderController');
        ...
        try {
            ...
            $disable_payments = [1,7];
            if(app('general_setting')->send_digital_product == 1 && $order->order_payment->status == 1 && !in_array($order->order_payment->payment_method,$disable_payments))
            {
                $onlyDigitalItem = true;
                foreach($order->packages as $packages) {
                    foreach($packages->products as $product){

                        if($product->seller_product_sku->product->product->is_physical == 0) {

                            DB::beginTransaction();
                            $order_data = [
                                'customer_id' => !empty($order->customer_id) ? $order->customer_id:null,
                                'seller_id' => $product->seller_product_sku->product->user_id,
                                'order_id' => $order->id,
                                'package_id' => $packages->id,
                                'seller_product_sku_id' => $product->seller_product_sku->id,
                                'product_sku_id' => (int) $product->seller_product_sku->product_sku_id,
                                'mail' => $order->customer_email,
                                "qty" => $product->qty
                            ];
                            $response = $this->ordermanageService->sendDigitalFileAccess(  $order_data);
                            if ($response == false) {
                                DB::rollBack();
                            } else {
                                DB::commit();
            ...
            // Corregir is_paid si el payment está pendiente (status=0)
            if($order->order_payment && $order->order_payment->status == 0){
                $order->is_paid = 0;
                $order->save();

                // Actualizar packages también
            ...
            if ($order->customer_id) {
                app(EntrepreneurPlanService::class)->runUpgrade($order->customer_id);
            }

            Toastr::success(__('order.order_created_successfully'), __('common.success'));
            LogActivity::successLog(__('order.order_created_successfully'));
    ...
    public function payment(Request $request)
    {
        ...
        }elseif ($payment_method == "EPAYCO") {
            $data['gateway_id'] = encrypt(16);
            $epaycoController = new EpaycoController();
            $epaycoController->payment($request->all());
            return;
        }
    ...
    public function my_purchase_order_pdf($id)
    {
        try {
            /**
             * #TODO: Ajustar la obtención de la marca (Brand).
             * Obtención dinámica de la marca (Brand) asociada a la orden.
             * Se debe modificar para identificar la marca específica asociada a los productos de la orden ($order).
             * Considerar casos donde una orden pueda tener múltiples marcas o requerir una marca principal
             * para la configuración de facturación DIAN.
             */
            $order = $this->orderService->orderFindByID(decrypt($id));

            // Intenta obtener la marca del primer producto en el primer paquete del pedido.
            $brand = $order->packages->first()?->products->first()?->seller_product_sku?->product?->product?->brand;

            // Si no se encuentra una marca, se usa la primera de la base de datos como fallback.
            if (!$brand) {
            ...
            LogActivity::errorLog(__('common.error_generating_pdf') . ': ' . $e->getMessage());
            return redirect()->back()->with('error', __('common.error_generating_pdf'));
        }
    }
    ...
    public function my_purchase_order_cancel(Request $request)
    {
        ...
        try {
            ...
            if (auth()->id() == $data->customer_id && $data->is_confirmed != 1) {
                ...
                LogActivity::successLog('Orden cancelada por ' . auth()->user()->first_name);
    ...
    public function my_purchase_order_package_cancel(Request $request)
    {
        ...
        try {
            ...
            if ($data->order->is_cancelled == 0) {
                ...
                LogActivity::successLog('Orden cancelada correctamente');
            } else {
