...
use App\Http\Resources\DeliveryProcessResource;
...
class OrderController extends Controller
{
    ...
    public function allOrderList(Request $request){
        ...
        if(count($orders) > 0){
            $orders = \App\Http\Resources\Api\v1\Orders\PendingOrderListResource::collection($orders);
    ...
    public function orderToShip(Request $request){
        ...
        if(count($packages) > 0){
             $packages = \App\Http\Resources\Api\v1\Orders\ReceivedOrdersResource::collection($packages);
            return response()->json([
                'packages' => $packages,
                'message' => 'success'
            ],200);
    ...
    public function InAppOrderStore(Request $request)
    {
        ...
        $order = $this->orderService->orderStoreForInAppPurchse($request->user(), $request->except('_token'));
        if($order)
        {
            return response()->json([
                "status" => 'success',
    ...
    public function refundOrderList(Request $request){
        ...
        $refundOrders = OrderRefundResource::collection($refundOrders);
    ...
    public function orderTrack(Request $request){
        ...
        try {
            ...
            if ($data['order'] == "Invalid Tracking ID") {
                ...
                return response()->json([
                    'message' => trans('app.Invalid Tracking ID') //$data['order']
                ],200);
            }
            elseif ($data['order'] == "Invalid Secret ID") {
                return response()->json([
                    'message' => trans('app.Invalid Secret ID')  //$data['order']
                ],200);
            }
            elseif ($data['order'] == "Phone Number didn't match") {
                return response()->json([
                    'message' => trans("app.phone Number didn't match")  //$data['order']
                ],409);
            }
            else {
                return response()->json([
    ...
    public function waitingForReview(Request $request){
        ...
        if(count($packages) > 0){
            return response()->json([
                'packages' => !empty($packages) ?  \App\Http\Resources\Api\v1\Orders\ReceivedOrdersResource::collection($packages):null,
                'message' => 'success'
            ],200);
        }else{
            return response()->json([
    ...
    public function reviewList(Request $request){
        ...
        if(count($reviews) > 0){
            return response()->json([
                'reviews' => !empty($reviews) ?  \App\Http\Resources\Api\v1\Orders\OrderReviewResource::collection($reviews):null,
                'message' => 'success'
            ],200);
        }else{
            return response()->json([
    ...
    {
       try{
            $deliveryStatus = DeliveryProcess::all();
            $deliveryStatus = DeliveryProcessResource::collection($deliveryStatus);
            return response()->json([
                "data" => $deliveryStatus,
    ...
    public function orderByDeliveryStatus(Request $request)
    {
        $data = $request->all();
       // $packages = $this->orderService->orderByDeliveryStatus($data, $request->user()->id);
        ...
        })->where('is_cancelled',0)->latest()->get();



        $orders = \App\Http\Resources\Api\v1\Orders\MyOrderResource::collection($packages);
