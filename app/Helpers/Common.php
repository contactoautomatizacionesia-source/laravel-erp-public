...
if (! function_exists('club_point_price')) {
    /**
     * Calculates the estimated monetary value of a given number of club points.
     * wallet_point represents the monetary value of 1 point.
     */
    function club_point_price(float $points): float
    {
        $walletPoint = \Modules\ClubPoint\Entities\ClubPointWallet::first();
        if (!$walletPoint || !$walletPoint->wallet_point) {
            return 0;
        }
        return $points * $walletPoint->wallet_point;
    }
}
...
if(!function_exists('hasBusinessInfo')){
    function hasBusinessInfo(){
        $business_info = DB::table('seller_business_information')->where('user_id',auth()->user()->id)->first();
        if(!empty($business_info) && $business_info->business_owner_name != '' &&  $business_info->business_address1 != '' && $business_info->business_country != '' && $business_info->business_state != '' ){
            return true;
        }
        return false;
    }
...
if(!function_exists('sellerPendingEarning')) {
    function sellerPendingEarning()
    {
        $amount = 0;
        $seller = getParentSeller()->sellerAccount;
        $ids= [];
        $shipping_cost = 0;
        $orders = OrderPackageDetail::where('seller_id', $seller->user_id)->where('delivery_status','!=',5)->select(['shipping_cost','order_id'])->get();

        foreach ($orders as $key => $order) {
            $ids[$key]=$order->order_id;
            $shipping_cost+= $order->shipping_cost;
        }
        if($seller->seller_commission_id == 1)
        {
            $product_amount = OrderProductDetail::whereIn('package_id', $ids)->sum('total_price');
            $commission_rate = $seller->commission_rate;
            $comission = ($product_amount * $commission_rate) / 100;

            $seller_amount  = $product_amount - $comission;
        }elseif($seller->seller_commission_id == 2){

        }else{
            $seller_amount = OrderProductDetail::whereIn('package_id', $ids)->sum('total_price');
        }


        return $seller_amount + $shipping_cost;
...
if (!function_exists('showImage')) {
    ...
    {
        $default_img = asset(asset_path('images/image_not_found.png'));
        if($path){
            if(strpos($path, 'amazonaws.com') != false || strpos($path, 'digitaloceanspaces.com') != false || strpos($path, 'drive.google.com') != false || strpos($path, 'wasabisys.com') != false || strpos($path, 'backblazeb2.com') != false || strpos($path, 'dropboxusercontent.com') != false || strpos($path, 'b-cdn.net') != false || strpos($path, 'contabostorage.com') != false){
                ...
                if(strpos($path, 'drive.google.com')){
                    $qs = getQueryParams($path);
                    return 'https://lh3.google.com/u/0/d/'.$qs['id'];
                }else{
                    return $path;
                }
            }
...
if (!function_exists('label_case_custom')) {
    function label_case_custom($text)
    {
        $smallWords = app()->getLocale() === 'es'
            ? ['de','la','el','y','con','para','por','en','del']
            : [
                'a','an','and','as','at','but','by','for','from',
                'in','into','nor','of','on','onto','or','over',
                'per','the','to','up','via','with'
            ];
...
if (!function_exists('digital_file_url')) {
    /**
     * Genera la URL para servir el archivo, eligiendo la ruta adecuada
     * según si el usuario es Admin o Cliente.
     */
    function digital_file_url($path)
    {
        if (empty($path)) return null;
        
        if (auth()->check() && in_array(auth()->user()->role->type, ['superadmin', 'admin', 'staff'])) {
            return route('admin.file-explorer.files.serve_path', ['path' => $path]);
        }

        return route('frontend.file-explorer.files.serve_path', ['path' => $path]);
    }
}

if (!function_exists('old_bool')) {
    function old_bool($key, $modelValue = null)
    {
        $value = old($key, $modelValue);
...
if (!function_exists('normalizeHex')) {
...
if (!function_exists('darkenColor')) {
    function darkenColor($hex, $amount)
    {
        ...
        $g = hexdec(substr($clean, 2, 2));
        $b = hexdec(substr($clean, 4, 2));
        $factor = 1 - min(max($amount, 0), 0.8);
        $dr = max(0, (int)floor($r * $factor));
        $dg = max(0, (int)floor($g * $factor));
        $db = max(0, (int)floor($b * $factor));
        return sprintf("#%02x%02x%02x", $dr, $dg, $db);
    }
}
if (!function_exists('formatNumberPoints')) {
    function formatNumberPoints($number)
    {
        $number = (float) $number;
        if ($number >= 1000000) {
