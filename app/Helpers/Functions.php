...
if(!function_exists('image_resize')){
    function image_resize(){
        ...
        $img = Image::make('https://amazcart.com/public/uploads/blog/026649a94d244f70d1ce3b08a5a801dd.jpg');
        $img->resize(320, 200);
        return $img;
    }
 }
...
function spn_active_link($route_or_path, $class = 'mm-active')
{
    ...
    if (is_array($route_or_path)) {
        ...
        return ($currentRouteName && in_array($currentRouteName, $route_or_path)) ? $class : false;
    } else {
        if ($currentRouteName === $route_or_path) {
            return $class;
        }
...
if(!function_exists('getGstName'))
{
    function getGstName($id = null)
    {
        $name = '';
        if($id !== null)
        {
            $gst = DB::table('gst_taxes')->where('id',$id)->first();
            if($gst)
            {
                $name = $gst->name;
            }
...
// Helper Genericos
/**
 * Devuelve una clase CSS según el ancho visual de un texto numérico.
 * La evaluación se basa en la cantidad de caracteres renderizados
 * (números, símbolos, separadores, decimales, etc.).
 *
 * Ejemplos válidos:
 * - "5"
 * - "19"
 ...
 * - "12%"
 *
 * @param string|int|float $displayValue Valor EXACTO que se muestra en la UI.
 * @return string Clase CSS correspondiente al tamaño visual.
 */
function visualSizeClass($displayValue)
{
    // Convertimos a string y limpiamos espacios
    $text = trim((string) $displayValue);

    // Longitud visual (multibyte safe)
...
if (!function_exists('getMailFromByProtocol')) {
    function getMailFromByProtocol(): string
    {
        $protocol = app('general_setting')->mail_protocol ?? 'smtp';

        $fromAddress = match($protocol) {
            'smtp', 'sendmail' => config('mail.from.address'),
            'sendgrid' => config('mail.mailers.sendgrid.email'),
            default    => config('mail.from.address'),
        };

        return $fromAddress ?? config('mail.from.address');
    }
}

if (!function_exists('translateWithFallback')) {
    /**
     * Translate a key with fallback to the original text if no translation exists.
     *
