...
use App\Services\Orders\ResolveOrderCostCenter;
...
class OrderRepository
{
    ...
    public function orderStore($data)
    {
        ...
        $cost_center_id = app(ResolveOrderCostCenter::class)->execute($delivery_type, $pickup_location, $shipping_address);
        $order = new Order();
        $order->customer_id =(auth()->check()) ? auth()->user()->id : null;
        if(isModuleActive('ClubPoint')){
            $order->club_point = $club_point;
            $order->total_points = $total_points;
            $walletRate = ClubPointWallet::value('wallet_point') ?? 0;
            $order->point_value = $club_point * $walletRate;
        }
        ...
        if(isModuleActive('MultiVendor')){
            foreach ($productList as $key => $products) {
                ...
                $package = OrderPackageDetail::create([
                    'order_id' => $order->id,
                    'seller_id' => $seller_id,
                    'package_code' => 'TRK-'.date('ymdhsi').'-'.Str::random(6),
                ...
                foreach ($products as $key => $product) {
                    ...
                    if(isModuleActive('AuctionProducts') && Session::has('auction_type')){
                            OrderProductDetail::create([
                                ...
                                'tax_amount' => $tax,
                                'unit_club_point' => (float) ($product->product->product->product->club_point ?? 0),
                            ]);
                    }else{
                        OrderProductDetail::create([
                            'package_id' => $package->id,
            ...
            if ($user != null) {
                ...
                foreach ($products as $key => $product) {
                    ...
                    OrderProductDetail::create([
                        ...
                        'tax_amount' => $tax,
                        'unit_club_point' => ($product['product_type'] == 'product') ? (float) ($product->product->product->club_point ?? 0) : 0,
                    ]);
                }
            } else {
