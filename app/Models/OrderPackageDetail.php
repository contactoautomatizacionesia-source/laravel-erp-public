...
class OrderPackageDetail extends Model
{
    ...
    public function formattedShippingDate(): string
    {
        ...
        $datePrefix = 'Est arrival date:';
        $timePrefix = 'Est arrival time:';

        if (!empty($shippingDate)) {
            if (Str::startsWith($shippingDate, $datePrefix)) {
                $datePart = trim(Str::after($shippingDate, $datePrefix));

                try {
                    $formattedShippingDate = __('shipping.estimated_arrival_date') . ': ' . Carbon::parse($datePart)->translatedFormat('d M Y');
                    ...
                    $datePart = str_replace(array_keys($monthMap), array_values($monthMap), $datePart);
                    $formattedShippingDate = __('shipping.estimated_arrival_date') . ': ' . $datePart;
                }
            } elseif (Str::startsWith($shippingDate, $timePrefix)) {
                $formattedShippingDate = Str::replaceFirst($timePrefix, __('shipping.estimated_arrival_time') . ':', $shippingDate);
            } else {
                $formattedShippingDate = $shippingDate;
            }
        }
    ...
    public function getDeliveryStateNameAttribute()
    {
        ...
        $deliveryStates = [
            2 => __('common.processing'),
            3 => __('common.shipped'),
            4 => __('common.received'),
        ];

        return $deliveryStates[$this->delivery_status] ?? __('common.delivered');
    }
