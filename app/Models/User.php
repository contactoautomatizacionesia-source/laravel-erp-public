...
class User extends Authenticatable
{
    ...
    use HasFactory, Notifiable, HasApiTokens, SendMail, SoftDeletes; // Activa la eliminación pasiva
    ...
    public function getCustomerCurrentWalletAmountsAttribute()
    {
        ...
        $club_point = $this->wallet_balances->where('status',1)->where('type', 'point')->sum('amount');
        $refund_back = $this->wallet_balances->where('status',1)->where('type', 'Refund Back')->sum('amount');
        $expensed = $this->wallet_balances->where('status',1)->where('type', 'Cart Payment')->sum('amount');
        $referral = $this->wallet_balances->where('status',1)->where('type', 'Referral')->sum('amount');
        $entryAmount = $this->wallet_balances->where('status',1)->where('type', 'Entry Amount Payment')->sum('amount');
        $rest_money = $deposite + $refund_back - $expensed - $withdraw + $club_point + $referral - $entryAmount;
    ...
    public function sendPasswordResetNotification($token)
    {
        ...
        $body = $tamplate->value;
        $key = ['https://{RESET_LINK}','{RESET_LINK}','{WEBSITE_NAME}','{EMAIL_SIGNATURE}'];
        ...
        $array['from'] = getMailFromByProtocol();
        $array['content'] = $body;
    ...
    public function getLastRehcargeAttribute(){
        return WalletBalance::with('user')->where('user_id', auth()->user()->id)->latest()->take(3)->get();
    }
    ...
    {
        return Folder::createUserFolder($this);
        // return Folder::where('owner_id', $this->id)
        //     ->where('type', 'user')
        //     ->first();
    }
    ...
    public function customerFinancialProfile()
    {
        return $this->hasOne(CustomerFinancialProfile::class, 'user_id');
    }
    ...
    public function dataUpdateLogs()
    {
        return $this->hasMany(DataUpdateLog::class, 'user_id');
    }
