(function ($) {
	...
	/*-------------------------------------------------------------------------------
         Nice Select
	   -------------------------------------------------------------------------------*/
    var getNiceSelectSearchPlaceholder = function ($select) {
        var customPlaceholder = $select && $select.length ? $select.data('search-placeholder') : null;
        ...
        if (typeof window.trans === 'function') {
            var translatedPlaceholder = window.trans('common.search');

            if (translatedPlaceholder && translatedPlaceholder !== 'common.search') {
    ...
    var applyNiceSelectSearchPlaceholder = function (context) {
        var $context = context ? $(context) : $(document);
        var $searchInputs = $context.hasClass('nice-select')
            ? $context.find('.nice-select-search')
            : $context.find('.nice-select .nice-select-search');

        $searchInputs.each(function () {
            var $searchInput = $(this);
            var $select = $searchInput.closest('.nice-select').prev('select');

            $searchInput.attr('placeholder', getNiceSelectSearchPlaceholder($select));
        });
    };
    ...
    applyNiceSelectSearchPlaceholder();

    $(document).on('click.nice_select_placeholder', '.nice-select', function () {
        applyNiceSelectSearchPlaceholder(this);
    });
...
function initTextareaCounters() {
	...
	textareas.forEach(textarea => {
		...
		const error = textarea.parentElement.querySelector('.error-text-counter');
        ...
        if (body.classList.contains('modal-open')) {
			...
			setTimeout(() => {
				...
				openModals.forEach(modal => {
					const elements = modal.querySelectorAll('.animate');
            ...
            document.querySelectorAll('.modal').forEach(modal => {
                modal.querySelectorAll(`.${ANIMATION_CLASS}`).forEach(el => {
