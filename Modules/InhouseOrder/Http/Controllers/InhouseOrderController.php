...
class InhouseOrderController extends Controller
{
    ...
    public function getData(){
        if(isset($_GET['table'])){
            ...
            return DataTables::of($order)
                ...
                ->addColumn('total_points', function($order){
                    return $order->total_points > 0 ? $order->total_points : '-';
                })
                ->addColumn('origin', function($order){
                    return $order->origin ?? '-';
                })
                ->addColumn('payment_method', function($order){
                    return $order->method?->method ?? '-';
                })
                ->addColumn('cost_center', function($order){
                    return 'Armenia';
    ...
    public function create(){
        try{
            session()->forget('inhouse_representative_id');
    ...
    public function store(Request $request){
        ...
        try{
            ...
            LogActivity::successLog(__('common.order_created_successfully'));
    ...
    public function getProductVariant(Request $request){
        ...
        try{
            ...
            if($product_type == 'variant_product'){
                ...
                $customer = null;
                $representative_id = $request->representative_id ?? session('inhouse_representative_id');

                if ($representative_id) {
                    session(['inhouse_representative_id' => $representative_id]);
                    $customer = \App\Models\User::with(['customerProfile','customerShippingAddress.getCountry','customerShippingAddress.getState','customerShippingAddress.getCity'])->find($representative_id);
                }

                return response()->json([
                    'modalData' =>  (string)view('inhouseorder::inhouse_order.components._product_variant_modal', compact('product','rating','total_review','customer')),
                    'productType' =>  $product_type
                ],200);
            }else{
                // Producto simple: también abre la modal
                session()->forget('item_details');
                $product = $this->inhouseOrderService->getVariantByProduct($request->product_id);
                ...
                return response()->json([
                    'modalData' => (string)view('inhouseorder::inhouse_order.components._product_variant_modal',
                                                compact('product', 'rating', 'total_review', 'customer')),
    ...
    public function saveRepresentative(Request $request){
        session(['inhouse_representative_id' => $request->representative_id]);
        return response()->json(['ok' => true]);
    }

    public function getCustomerInfo(Request $request){
        try{
            $customer = \App\Models\User::with(['customerProfile','customerShippingAddress.getCountry','customerShippingAddress.getState','customerShippingAddress.getCity'])
                ->findOrFail($request->customer_id);

            $shippingAddr = $customer->customerShippingAddress;

            return response()->json([
                'name'       => trim($customer->first_name . ' ' . $customer->last_name),
                'email'      => $customer->email,
                'phone'      => $customer->customerProfile?->whatsapp ?? $customer->phone ?? '',
                'code'       => $customer->customerProfile?->code ?? '',
