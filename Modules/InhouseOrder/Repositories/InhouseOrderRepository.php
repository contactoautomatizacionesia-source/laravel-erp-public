...
class InhouseOrderRepository
{
    ...
    public function store($data){
        ...
        $representative_id = session()->get('inhouse_representative_id');
        ...
        $order = Order::create([
            'customer_id' => $representative_id ?: null,
            ...
            'total_points' => (float) ($data['total_points'] ?? 0)
        ]);
        ...
        foreach($grouped as $key => $packages){
            ...
            $packageData = OrderPackageDetail::create([
                'order_id' => $order->id,
                'seller_id' => $key,
                'package_code' => 'TRK-' . Str::upper(Str::random(12)),
