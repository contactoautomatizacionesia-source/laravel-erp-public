                                    ...
                                    <div class="QA_table ">
                                        <div class="overflow-auto min-height-280">
                                            <table class="table">
                                                <thead>
                                                    <tr>
                                                        <th width="">{{ __('product.product_name') }}</th>
                                                        <th width="">{{ __('product.variation') }}</th>
                                                        <th width="">{{ __('common.points') }}</th>
                                                        <th width="">{{ __('common.price') }}</th>
                                                        <th width="">{{ __('common.qty') }}</th>
                                                        <th width="">{{ __('common.total_price') }}</th>
                                                        <th width="">{{ __('common.remove') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @php
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            {{ $item->product->product->product->club_point ?? '-' }}
                                                        </td>
                                                        <td class="text-nowrap">
                                                            ...
                                                            @php
                                                                ...
                                                                $totalPoints += ($item->product->product->product->club_point ?? 0) * $item->qty;
                                                            @endphp
...
<div class="col-lg-4 mb-50 ml-auto"> 
    <table class="direction_table table-borderless clone_line_table">
        <tr>
            <div class="main-title d-flex">
        ...
        <tr class="border-bottom">
            ...
            <td class="text-end py-2 "> <strong class="pl-4">{{getNumberTranslate($totalItem)}}</strong></td>
            <input type="hidden" value="{{$totalItem}}" name="total_quantity">
        </tr>
        <tr class="border-bottom">
            <td>{{ __('order.sub_total') }}</strong></td>
            <td class="text-end py-2 "> <strong class="pl-4">{{single_price($subtotal)}}</strong></td>
            <input type="hidden" value="{{$subtotal}}" name="sub_total">
        </tr>
        <tr class="border-bottom">
            <td>{{ __('common.discount') }}</td>
            <td class="text-end py-2 "> <strong class="pl-4">- {{single_price($subtotal - $actualtotal)}}</strong></td>
            <input type="hidden" value="{{$subtotal - $actualtotal}}" name="discount_total">
        </tr>
        <tr class="border-bottom">
            <td>{{ __('order.total_shipping_charge') }}</td>
            <td class="text-end py-2 "> <strong class="pl-4">{{single_price($shippingtotal)}}</strong></td>
            <input type="hidden" value="{{$shippingtotal}}" name="shipping_charge">
        </tr>
        <tr class="border-bottom">
            <td>{{ __('Total TAX/GST') }}</td>
            <td class="text-end py-2 "> <strong class="pl-4">{{single_price($gstAmountTotal)}}</strong></td>
            <input type="hidden" value="{{$gstAmountTotal}}" name="gst_tax_total">
        </tr>
        <tr class="border-bottom">
            <td>{{ __('common.grand_total') }}</td>
            <td class="text-end py-2 "> <strong class="pl-4">{{single_price($actualtotal + $shippingtotal  + $gstAmountTotal)}}</strong></td>
            <input type="hidden" value="{{$actualtotal + $shippingtotal  + $gstAmountTotal}}" name="grand_total">
        </tr>
        <tr class="border-bottom">
            <td>{{ __('common.totals_points') }}</td>
            <td class="text-end py-2 "> <strong class="pl-4">{{getNumberTranslate($totalPoints)}}</strong></td>
            <input type="hidden" value="{{$totalPoints}}" name="total_points">
        </tr>
    </table>
</div>
