...
@endsection
@section('mainContent')
<section class="admin-visitor-area up_st_admin_visitor ign-in-house-order-create">
    <div class="container-fluid p-0">
        <div class="main-title d-md-flex align-items-baseline gap-2">
            <x-backEnd.back-button :text="false" />
            <h3 class="mb-30">{{__('order.create_new_order')}} </h3>
        </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="primary_input mb-25">
                                                ...
                                                <label class="primary_input_label">{{label_case_custom(__('common.representative'))}}</label>
                                                <select 
                                                    name="representative" 
                                                    id="representative" 
                                                    data-validate="required" 
                                                    class="nice-select-ajax  wide mb-25 "
                                    ...
                                    <div class="col-lg-12">
                                        <div class="primary_input mb-25">
                                            <label class="primary_input_label" for="product">{{ __('product.product_list') }}<span class="text-danger">*</span></label>
                                            <select class="primary_select mb-25" name="product" id="product">
                                                ...
                                                @if(isModuleActive('MultiVendor'))
                                                    @foreach($products as $key => $product)
                                                        <option value="{{$product->id}}">{{ substr($product->product_name,0,35) }} @if(strlen($product->product_name) > 35)... @endif [@if($product->seller->role->type == 'seller'){{$product->seller->first_name}} @else {{__('common.inhouse')}} @endif]</option>
                                                    @endforeach
                                                @else
                                    ...
                                    <div class="col-lg-12">
                                        <div class="primary_input mb-25">
                                            <label class="primary_input_label" for="payment_method">{{ __('common.payment_method') }} <span class="text-danger">*</span></label>
                                            <select class="primary_select mb-25" name="payment_method" id="payment_method">
                                                <option value="{{$paymentMethod->id}}">{{__("payment_gatways.cash_on_delivery")}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <div class="primary_input mb-25">
                                            <label class="primary_input_label" for="origin">{{ __('common.origin') }}</label>
                                            <input class="primary_input_field" type="text" name="origin" id="origin" placeholder="{{ __('common.origin') }}">
                                        </div>
                                    </div>
                                    @if(\Session::has('inhouse_order_shipping_address'))
                                    @else
                                    <div class="col-lg-12" id="alert_div">
                                        <div class="alert alert-warning mt-30 text-center">{{ __('shipping.please_save_customer_address_before_add_product') }}
    ...
    <script>
        (function($){
            ...
            $(document).ready(function(){
                ...
                $(document).on('change', '#representative', function(event){
                    let representative_id = $('#representative').val();
                    $('#pre-loader').removeClass('d-none');
                    if(representative_id){
                        // Precargar datos del empresario en el formulario de dirección
                        let customerInfoUrl = base_url + '/admin/in-house-order/get-customer-info?customer_id=' + representative_id;
                        $.get(customerInfoUrl, function(data) {
                            if (!data.error) {
                                ...
                                if (data.country) {
                                    ...
                                    // Cargar estados y luego preseleccionar state/city
                                    if (data.state) {
                                        let stateUrl = base_url + '/seller/profile/get-state?country_id=' + data.country;
                                        $.get(stateUrl, function(states) {
                                            $('#shipping_state').empty().append('<option value="" disabled selected>{{__("common.select_one")}}</option>');
                                            $.each(states, function(i, s) {
                                                $('#shipping_state').append('<option value="' + s.id + '">' + s.name + '</option>');
                                            });
                                            $('#shipping_state').val(data.state).trigger('change');
                                            $('#shipping_state').niceSelect('update');
                                            // Cargar ciudades
                                            if (data.city) {
                                                let cityUrl = base_url + '/seller/profile/get-city?state_id=' + data.state;
                                                $.get(cityUrl, function(cities) {
                                                    $('#shipping_city').empty().append('<option value="" disabled selected>{{__("common.select_one")}}</option>');
                                                    $.each(cities, function(i, c) {
                                                        $('#shipping_city').append('<option value="' + c.id + '">' + c.name + '</option>');
                                                    });
                                                    $('#shipping_city').val(data.city).niceSelect('update');
                                                });
                                            }
                                        });
                                    }
                                    $('#shipping_country').niceSelect('update');
                                }
                            }
                        });
                        // Guardar representative_id en sesión para que la modal lo use al seleccionar producto
                        let saveRepUrl = base_url + '/admin/in-house-order/save-representative?representative_id=' + representative_id;
                        $.get(saveRepUrl).always(function() {
                ...
                $(document).on('change', '#product', function(event){
                    ...
                    if(product_id){
                        let representative_id = $('#representative').val();
                        let url = base_url + '/admin/in-house-order/get-product-variant?product_id=' + product_id + (representative_id ? '&representative_id=' + representative_id : '');
                        $.get(url, function(data){
                            if(data.productType == 'variant_product' || data.productType == 'single_product'){
                                $('#productVariantDiv').html(data.modalData);
                                $('#theme_modal').modal('show');
                                $('#shipping_type').niceSelect();
                            }else{
                                if(data == 'out_of_stock'){
                                    toastr.error(trans('defaultTheme.out_of_stock'));
                                }else{
                ...
                $(document).on('click', '#save_address_btn', function(event){
                    $('#pre-loader').removeClass('d-none');
                    let post_code_required = "{{$postcodeRequired}}";
                    ...
                    let error_show = 0;
                    if(shipping_name == ''){
                        $('#error_shipping_name').text(trans('validation.required', {attribute: trans('common.name')}));
                        error_show = 1;
                    }
                    if(shipping_email == ''){
                        $('#error_shipping_email').text(trans('validation.required', {attribute: trans('common.email')}));
                        error_show = 1;
                    }
                    if(shipping_phone == ''){
                        $('#error_shipping_phone').text(trans('validation.required', {attribute: trans('common.phone')}));
                        error_show = 1;
                    }
                    if(shipping_address == ''){
                        $('#error_shipping_address').text(trans('validation.required', {attribute: trans('common.address')}));
                        error_show = 1;
                    }
                    if(shipping_country == null){
                        $('#error_shipping_country').text(trans('validation.required', {attribute: trans('common.country')}));
                        error_show = 1;
                    }
                    if(shipping_state == null){
                        $('#error_shipping_state').text(trans('validation.required', {attribute: trans('common.state')}));
                        error_show = 1;
                    }
                    if(shipping_city == null){
                        $('#error_shipping_city').text(trans('validation.required', {attribute: trans('common.city')}));
                        error_show = 1;
                    }
                    if(shipping_postcode == '' && post_code_required){
                        $('#error_shipping_postcode').text(trans('validation.required', {attribute: trans('common.postcode')}));
                        error_show = 1;
                    }
                    if(same_address == 0){
                        if(error_show == 1){
                    ...
                    if(same_address == 1){
                        if(billing_name == ''){
                            $('#error_billing_name').text(trans('validation.required', {attribute: trans('common.name')}));
                            error_show = 1;
                        }
                        if(billing_email == ''){
                            $('#error_billing_email').text(trans('validation.required', {attribute: trans('common.email')}));
                            error_show = 1;
                        }
                        if(billing_phone == ''){
                            $('#error_billing_phone').text(trans('validation.required', {attribute: trans('common.phone')}));
                            error_show = 1;
                        }
                        if(billing_address == ''){
                            $('#error_billing_address').text(trans('validation.required', {attribute: trans('common.address')}));
                            error_show = 1;
                        }
                        if(billing_country == null){
                            $('#error_billing_country').text(trans('validation.required', {attribute: trans('common.country')}));
                            error_show = 1;
                        }
                        if(billing_state == null){
                            $('#error_billing_state').text(trans('validation.required', {attribute: trans('common.state')}));
                            error_show = 1;
                        }
                        if(billing_city == null){
                            $('#error_billing_city').text(trans('validation.required', {attribute: trans('common.city')}));
                            error_show = 1;
                        }
                        if(billing_postcode == '' && post_code_required){
                            $('#error_billing_postcode').text(trans('validation.required', {attribute: trans('common.postcode')}));
                            error_show = 1;
                        }
                        if(error_show == 1){
                            $('#pre-loader').addClass('d-none');
