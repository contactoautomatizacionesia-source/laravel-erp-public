...
class SidebarManagerController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $backendMenuUser = BackendmenuUser::where('user_id', auth()->id())
                ->hasValidMenu()
                ->get(['id', 'backendmenu_id']);
            ...
            $data['unused_menus'] = BackendmenuUser::with('backendMenu')->where('user_id', auth()->id())->hasValidMenu()->where('status', 0)->get();
            $data['backendMenuUser'] = BackendmenuUser::with('children', 'backendMenu')->whereNull('parent_id')->where('user_id', auth()->id())->hasValidMenu()->orderBy('position')->get();
    ...
    private function reloadWithData(){
        $backendMenuUser = BackendmenuUser::with('children', 'backendMenu')->whereNull('parent_id')->where('user_id', auth()->id())->hasValidMenu()->orderBy('position')->get();
        $unused_menus = BackendmenuUser::with('backendMenu')->where('user_id', auth()->id())->hasValidMenu()->where('status', 0)->get();
        return response()->json([
            'msg' => 'success',
