@push('scripts')
    <script type="text/javascript">
        (function($){
            "use strict";
            var cancelReasonExportTitle = @json(app('general_setting')->site_title . ' | ' . __('order.cancel_reason'));
            $(document).ready(function () {
                var baseUrl = $('#app_base_url').val();

                function applyCancelReasonTableLocalization() {
                    if (!$('#refund_process_list .Crm_table_active3').length) {
                        return;
                    }
                    $('#header_title').text(cancelReasonExportTitle);
                    document.title = cancelReasonExportTitle;
                ...
                @endif
                    ...
                    let formData = $(this).serializeArray().map(function(field) {
                        return { name: field.name, value: sanitizeInput(field.value) };
                    });
                    $.ajax({
                ...
                $("#refund_process_list").on("click", ".edit_reason", function () {
                    ...
                    @else
                        function getTranslatedValue(value, fallbackValue) {
                            if (value && typeof value === 'object') {
                                var userLang = '{{ auth()->user()->lang_code }}';
                                return value[userLang] || value['en'] || Object.values(value)[0] || fallbackValue;
                            }
                            return value || fallbackValue;
                        }

                        $(".name").val(getTranslatedValue(item.name, item.translateName));
                        $(".description").val(getTranslatedValue(item.description, item.TranslateDescription));
                    @endif
                        $(".edit_id").val(item.id);
                });
                $(document).on('click', '#cancelEditBtn', function () {
                    $("#processEditForm").trigger("reset");
                    $('[id^="edit_name_error"]').text('');
                    $('[id^="edit_description_error"]').text('');
                    $('.edit_div').hide();
                    $('.create_div').show();
                });
                ...
                applyCancelReasonTableLocalization();
