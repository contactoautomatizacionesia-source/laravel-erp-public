...
use Modules\OrderManage\Http\Requests\DeliveryProcessRequest;
...
class DeliveryProcessController extends Controller
{
    ...
    public function store(DeliveryProcessRequest $request)
    {
        try {
            ...
            return response()->json(["message" => "Nuevo proceso de entrega agregado exitosamente"], 200);
        } catch (\Exception $e) {
            LogActivity::errorLog($e->getMessage());
            return response()->json(["message" => "Algo salió mal", "error" => $e->getMessage()], 503);
        }
    }
    ...
    public function update(DeliveryProcessRequest $request)
    {
        try {
            ...
            return response()->json(["message" => "Proceso de entrega actualizado exitosamente"], 200);
        } catch (\Exception $e) {
            LogActivity::errorLog($e->getMessage());
            return response()->json(["message" => "Algo salió mal", "error" => $e->getMessage()], 503);
        }
