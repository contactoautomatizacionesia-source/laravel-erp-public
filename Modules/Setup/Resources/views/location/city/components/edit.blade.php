...
<div class="main-title">
    <h3 class="mb-20">
        {{__('common.edit')}} {{__('common.city')}}</h3>
</div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="primary_input mb-25">
                    ...
                    <input name="name" class="primary_input_field name"
                        id="name" placeholder="{{ __('common.name') }}" value="{{$city->name}}"
                        type="text">
                ...
                <div class="primary_input mb-25">
                    ...
                    <select name="country" id="country" class="primary_select mb-15">
                        ...
                        @foreach ($countries as $key => $country)
                            <option {{$city->state->country->id == $country->id?'selected':''}} value="{{ $country->id }}">{{ $country->name }}</option>
                        @endforeach
            ...
            <div class="col-lg-12" id="stateDiv">
                <div class="primary_input mb-25">
                    ...
                    <select name="state" id="state" class="primary_select mb-15">
                        <option value="" selected disabled>{{__('common.select_one')}}</option>
                        @foreach($city->state->country->states as $key => $state)
                            <option {{$city->state->id == $state->id?'selected':''}} value="{{$state->id}}">{{$state->name}}</option>
                        @endforeach
                    ...
                    <ul id="theme_nav" class="permission_list sms_list ">
                        <li>
                            <label data-id="bg_option"
                                ...
                                <input name="status" id="status_active" value="1" {{$city->status == 1?'checked':''}} class="active" type="radio">
                        ...
                        <li>
                            <label data-id="color_option"
                                   class="primary_checkbox d-flex mr-12">
                                <input name="status" value="0" id="status_inactive" {{$city->status == 0?'checked':''}} class="de_active"
                                       type="radio">
                                <span class="checkmark"></span>
                            </label>
