            ...
            <ul id="" class="permission_list sms_list">
                @foreach ($sms_gateways as $key => $smsGateway)
                    <li>
                        ...
                        <p>
                            @php
                                switch ($smsGateway->type) {
                                    case 'Twillo':
                                    echo __("general_settings.twillo");
                                    break;
                                    case 'TextLocal':
                                    echo __("general_settings.textlocal");
                                    break;
                                    case 'OtherSmsGateway':
                                    echo __("general_settings.othersmsgateway");
                                    break;
                                    case 'MsegatSMS':
                                    echo __("general_settings.msegatsms");
                                    break;
                                }
                            @endphp
                        </p>
        ...
        <div class="col-12">
            ...
            <div id="Twilio_Settings" class="sms_ption" >
                ...
                <div class="row">
                    <div class="col-xl-6">
                        <div class="primary_input mb-25">
                            ...
                            <input class="primary_input_field" placeholder="-" type="text" name="TWILIO_SID" value="{{ config('services.twilio.sid') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-xl-6">
                        <div class="primary_input mb-25">
                            ...
                            <input class="primary_input_field" placeholder="-" type="text" name="TWILIO_TOKEN" value="{{ config('services.twilio.token') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-xl-6">
                        <div class="primary_input mb-25">
                            ...
                            <input class="primary_input_field"
                                   ...
                                   value="{{ config('services.twilio.number') }}"
                ...
                <div class="submit_btn text-center mb-100 pt_15">
                    <button  name="action" value="twilo" class="btn-toolkit btn-primary" type="submit"> <i class="ti-check"></i> {{ __('common.save') }}</button>
                </div>
                <!-- content  -->
            </div>
                ...
                <div class="row">
                    <div class="col-xl-6">
                        <div class="primary_input mb-25">
                            ...
                            <input class="primary_input_field" placeholder="-" type="text" name="TEXT_TO_LOCAL_API_KEY" value="{{ config('services.textlocal.api_key') }}">
                        </div>
                    </div>
                    <div class="col-xl-6">
                        <div class="primary_input mb-25">
                            ...
                            <input class="primary_input_field" placeholder="-" type="text" name="TEXT_TO_LOCAL_SENDER" value="{{ config('services.textlocal.sender') }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-lg-3">
                        <div class="primary_input mb-15">
                            ...
                            <select class="primary_select mb-15" id="request_method" name="request_method">
                                ...
                                <option {{smsGatewaySetting()['request_method'] == 'GET' ? 'selected' :''}} value="GET">{{__("common.get")}}</option>
                                <option {{smsGatewaySetting()['request_method'] == 'POST' ? 'selected' :''}} value="POST">{{__('common.post')}}</option>
                            </select>
...
<form class="" action="{{ route('sms_send_demo') }}" method="post">
    ...
    <div class="row">
        <div class="col-xl-12">
            <div class="primary_input mb-25">
                <label class="primary_input_label" for="">{{ __('common.phone_number') }} <span class="text-danger">*</span></label>
                <input class="primary_input_field"
                       placeholder="-"
                       type="tel"
                       value="{{old('number')}}"
        ...
        <div class="col-xl-12">
            <div class="primary_input mb-25">
                <label class="primary_input_label" for="">{{ __('general_settings.send_a_test_sms') }} <span class="text-danger">*</span></label>
    ...
    <div class="submit_btn text-center mb-100 pt_15">
        <button class="btn-toolkit btn-primary" type="submit">{{ __('general_settings.send_test_sms') }}</button>
    </div>
</form>
...
<script>
    function formatPhoneNumber(input) {
        ...
        // 1. Si está vacío, no hacer nada
        if (value.length === 0) return;

        // 2. Limpiar caracteres no permitidos (solo números y +)
        // Esto previene que peguen letras o símbolos raros
        value = value.replace(/[^0-9+]/g, '');

        // 3. Si el primer carácter es un número (0-9), agregar el + al inicio
        if (/^[0-9]/.test(value)) {
            value = '+' + value;
        }

        // 4. Evitar que haya más de un signo + o que esté en una posición incorrecta
        // Si hay un + que no esté al principio, lo eliminamos
