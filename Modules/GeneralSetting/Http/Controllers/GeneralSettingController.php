...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->has('status')) {
            $request->validate([
                ...
                "site_logo_light" => "nullable|mimes:jpg,png,jpeg",
                "bg_footer" => "nullable|mimes:jpg,png,jpeg",
                ...
                "invoice_footer_quote" => "nullable|string",
                "product_report" => "product_report",
                "send_digital_product" => "nullable"
            ]);
        }
        ...
        if ($request->site_logo_light != null) {
            $url = $this->saveSettingsImage($request->site_logo_light,50, 193);
            $this->deleteImage(app('general_setting')->logo_light);
            $request->merge(["logo_light" => $url]);
        }
        if ($request->bg_footer != null) {
            $url = $this->saveSettingsImage($request->bg_footer,500, 1920);
            $this->deleteImage(app('general_setting')->footer_background);
            $request->merge(["footer_background" => $url]);
        }
        ...
        try {
            ...
            $setting = app('general_setting'); // ← get the setting instance

            $footerText = json_decode($setting->invoice_footer_text, true) ?? [];
            $footerText[$locale] = $request->input('invoice_footer_text', '');

            $footerQuote = json_decode($setting->invoice_footer_quote, true) ?? [];
            $footerQuote[$locale] = $request->input('invoice_footer_quote', '');

            $request->merge([
                'invoice_footer_text'  => json_encode($footerText),
                'invoice_footer_quote' => json_encode($footerQuote),
            ]);

            $this->generalSettingService->update($request->except("_token", "favicon_logo", "site_logo", "site_logo_light", "bg_footer", "shop_link_banner", "currency_id", "status"));
    ...
    public function update_activation_status(Request $request)
    {
        ...
        try {
            $this->generalSettingService->updateActivation($request->only('id', 'status'));
    ...
    public function sms_gateway_credentials_update(Request $request)
    {
        if($request->input('action') == 'other'){
            ...
            // Validaciones para los campos de twilio
            $request->validate([
                'sms_gateway_id' => 'required',
                'TWILIO_SID' => 'nullable|required_if:action,twilo',
                'TWILIO_TOKEN' => 'nullable|required_if:action,twilo',
                'VALID_TWILLO_NUMBER' => 'nullable|required_if:action,twilo|regex:/^\+[0-9]{7,15}$/'
            ]);
        }

        try {

            $this->generalSettingService->updateSmsActivation($request->except("_token"));
    ...
    public function smtp_gateway_credentials_update(Request $request)
    {
        ...
        try {
            $this->generalSettingService->updateSmtpGatewayCredential($request->except("_token"));

            session()->forget('g_set');
            session()->forget('sms_set');
            session()->put('smtp_set', '1');
            Toastr::success(__('common.updated_successfully'), __('common.success'));
            // Ajuste en el log para que sea dinámico
            LogActivity::successLog($request->mail_gateway . ' gateway credential successful.');
            return back();
        } catch (\Exception $e) {
            LogActivity::errorLog($e->getMessage());
    ...
    public function test_mail_send(Request $request)
    {
        ...
        try {
            $mail =  $this->sendMailTest($request->email, "Test Smtp Mail", $request->content);
            if ($mail === true) {
