                        ...
                        <div class="white_box_30px box_shadow_white">
                            <div class="box_header">
                                <div class="main-title d-flex">
                                    <h3 class="mb-0 mr-30">{{ __('common.bulk_customer_upload') }}</h3>
                                </div>
                            </div>
                            <form action="{{ route('admin.coustomer.bulkupload.store') }}" method="POST" enctype="multipart/form-data" class="csvForm">
                                @csrf
                                <div class="row form">
                                    <div class="col-xl-12 col-lg-12 col-md-12">
                                       <div class="primary_input mb-15">
                                          <label class="primary_input_label" for="">{{ __('common.csv_upload') }} <small><a class="d-flex float-right" href="{{ asset(asset_path('bulk_upload_sample/customer.xlsx')) }}" download>{{ __('common.sample_file_download') }}</a><small> </label>
