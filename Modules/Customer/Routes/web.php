...
Route::prefix('customer')->group(function () {
    ...
    Route::get('/check-availability', 'CustomerValidationController@checkAvailability')->middleware('throttle:60,1')->name('customer.check_availability');

    Route::get('/get-active-customers', 'CustomerController@getActiveCustomers')->name('get_active_customers');
    ...
    Route::post('/approval-status/update', 'CustomerController@update_approval_status')->name('customer.update_approval_status')->middleware(['auth', 'admin', 'prohibited_demo_mode']);
    // Acción de Restaurar
    Route::get('/restore/{id}', 'CustomerController@restore')->name('customer.restore')->middleware(['auth', 'admin', 'permission']);
    ...
    Route::post('/profile/request/change-plan', 'CustomerController@customerToChangePlan')->name('customer.show_customer_change_plan')->middleware(['auth', 'admin', 'permission']);
    Route::post('/profile/assign-plan', 'CustomerController@assignPlanManually')->name('customer.assign_plan_manually')->middleware(['auth', 'admin', 'permission']);
...
Route::prefix('admin')->middleware(['auth', 'admin'])->as('admin.')->group(function () {
    ...
    // [DEV] Preview contrato del último usuario registrado — eliminar cuando no se necesite
    Route::get('/customer/contract-preview', 'CustomerController@contractPreview')->name('customer.contract_preview')->middleware('auth');
});
Route::prefix('admin')->middleware(['auth', 'admin'])->as('admin.')->group(function () {
    Route::get('/customer/create', 'CustomerController@create')->name('customer.create')->middleware('permission');
    Route::post('/customer/store', 'CustomerController@store')->name('customer.store')->middleware('prohibited_demo_mode');
