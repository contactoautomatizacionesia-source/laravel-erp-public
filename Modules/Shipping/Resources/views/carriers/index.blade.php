...
<section class="admin-visitor-area up_st_admin_visitor">
    <div class="row mx-0">
        ...
        @endphp
        <div class="{{$activeAutomaticCarrier > 0 ? 'col-md-5 col-sm-6 col-xs-12': "col-md-12 col-sm-12 col-xs-12"}} white_box_30px">
...
(function($) {
    ...
    $(document).ready(function(){
        if ($.fn.DataTable && $.fn.DataTable.isDataTable('#carrierTable')) {
            let existingSettings = $('#carrierTable').DataTable().settings()[0];
            const hasLocalizedButtons = !!_.get(existingSettings, 'oLanguage.buttons.copyTitle');
            if (!hasLocalizedButtons) {
                $('#carrierTable').DataTable().destroy();
                initGlobalDataTable('#carrierTable');
            }
        }
        $(document).on('click','.disable_shiprocket',function (){
                toastr.info("{{ __('shipping.please_configure_shiprocket_first') }}", "{{ __('common.error') }}");
            })
        ...
        $(document).on('submit', '#create_form', function(event){
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    toastr.success("{{ __('shipping.carrier_add_successfully') }}");
                    $('#pre-loader').addClass('d-none');
        ...
        $(document).on('click', '.edit_carrier', function(event){
            ...
            $.get(url, function(response){
                if(response.msg_type == 'Manual'){
                    ...
                    toastr.error("{{ __('shipping.automatic_carriers_not_editable') }}", "{{ __('common.error') }}");
                }
                $('#pre-loader').addClass('d-none');
            });
        });
        ...
        $(document).on('submit', '#update_form', function(event){
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    $('#pre-loader').addClass('d-none');
                    toastr.success("{{ __('shipping.carrier_update_successfully') }}");
                },
                error:function(response) {
                    if(response.responseJSON.error){
                        toastr.error(response.responseJSON.error ,"{{ __('common.error') }}");
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.msg_type == 'Automatic'){
                        toastr.error("{{ __('shipping.automatic_carriers_delete_not_possible') }}", "{{ __('common.error') }}");
                    }
                    else if(response.msg_type == 'last_item'){
                        toastr.error("{{ __('shipping.last_carrier_delete_not_possible') }}", "{{ __('common.error') }}");
                    }
                    else if(response.msg_type == 'has_shipping_method'){
                        toastr.error("{{ __('shipping.this_carrier_added_on_shipping_rate') }}", "{{ __('common.error') }}");
                    }
                    else{
                        toastr.success("{{ __('shipping.deleted_successfully') }}", "{{ __('common.success') }}")
                    }
                    resetAfterChange(response.carrier_list,response.config);
                    $("#pre-loader").addClass('d-none');
                },
