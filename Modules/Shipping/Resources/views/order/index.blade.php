            ...
            <div class="row justify-content-center">
                <div class="col-lg-12 mb-25">
                    <div class="white_box_30px box_shadow_white">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12 text-center">
                                    <div class="d-flex justify-content-center">
                                        <button class="btn-toolkit btn-primary btn-icon mr-3"  type="submit"><i class="ti-search"></i>{{ __('common.search') }}</button>
                                        <a href="{{route('shipping.pending_orders.index')}}" class="btn-toolkit btn-secondary-outline btn-icon"  type="button"><i class="ti-reload"></i>{{ __('shipping.reset') }}</a>
                                    </div>
                                </div>
                            </div>
                        </form>
                        ...
                        <div class="QA_section QA_section_heading_custom check_box_table mt-5">
                            <div class="QA_table ">
                                <!-- table-responsive -->
                                <div class="">
                                    <table id="shippingOrderTable" class="table Crm_table_active3">
                                        <thead>
                                        <tr>
                                            <th>{{__('common.sl')}}</th>
                                        ...
                                        @foreach($orders as $key => $row)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($packaging_info)
                                                    {{getNumberTranslate($row->weight)}} - {{getNumberTranslate($row->length)}} x {{getNumberTranslate($row->breadth)}} x {{getNumberTranslate($row->height)}}
                                                    @endif
                                                </td>
                                                <td>
                                                    <div class="dropdown CRM_dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2">
                                                            @if (permissionCheck('shipping.label_generate'))
                                                                <a  target="_blank" href="{{route('shipping.label_generate',$row->id)}}" download="" class="dropdown-item">{{__('shipping.Print of Manifestation ')}}</a>
                                                            @endif
                                                            @if (permissionCheck('shipping.invoice_generate'))
                                                                <a download="" target="_blank" href="{{route('shipping.invoice_generate',$row->id)}}" class="dropdown-item">{{__('shipping.invoice')}}</a>
                                                            @endif
        ...
        (function ($) {
            ...
            $(document).ready(function () {
                if ($.fn.DataTable && $.fn.DataTable.isDataTable('.Crm_table_active3')) {
                    let settings = $('.Crm_table_active3').DataTable().settings()[0];
                    const hasLocalizedButtons = !!_.get(settings, 'oLanguage.buttons.copyTitle');
                    if (!hasLocalizedButtons && $.fn.DataTable.isDataTable('#shippingOrderTable')) {
                        $('#shippingOrderTable').DataTable().destroy();
                        initGlobalDataTable('#shippingOrderTable');
                    }
                }
                ...
                $(document).on('change', '#filter', function(event){
                    ...
                    $.each(couriers, function (index, c) {
                        data+= ` <li>
                                    ...
                                    <p>`+c.courier_name+` (Freight Charges: `+c.freight_charge+` , {{__('shipping.estimated_delivery')}}: `+c.estimated_delivery_days+` days)</p>
                                 </li>
                                `
                    });
