        ...
        $(document).on('submit', '#create_form', function(event){
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    toastr.success("Transportador agregado exitosamente");
        ...
        $(document).on('click', '.edit_carrier', function(event){
            ...
            $.get(url, function(response){
                if(response.msg_type == 'Manual'){
                    ...
                    toastr.error('Los transportadores automáticos no son editables.');
                }
                $('#pre-loader').addClass('d-none');
            });
        });
        ...
        $(document).on('submit', '#update_form', function(event){
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    $('#pre-loader').addClass('d-none');
                    toastr.success("Transportador actualizado exitosamente");
                },
                error:function(response) {
                    if(response.responseJSON.error){
                        toastr.error(response.responseJSON.error ,"Error");
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.msg_type == 'Automatic'){
                        toastr.error('No es posible eliminar transportadores automáticos.');
                    }
                    else if(response.msg_type == 'last_item'){
                        toastr.error('No es posible eliminar el último transportador.');
                    }
                    else if(response.msg_type == 'has_shipping_method'){
                        toastr.error('Este transportador está agregado en una Tarifa de Envío.');
                    }
                    else{
                        toastr.success("Eliminado exitosamente","Éxito")
                    }
                    resetAfterChange(response.carrier_list,response.config);
                    $("#pre-loader").addClass('d-none');
                ...
                error: function(response) {
                    ...
                    toastr.error("Mensaje de error","Error");
                }
            });
        });
