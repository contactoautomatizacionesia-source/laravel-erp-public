...
use Modules\Shipping\Exceptions\CannotActivatePickupLocationException;
...
class PickupLocationRepository
{
    public function all()
    {
        ...
        $pickupLocations = $this->visibleQuery($user_id)->get();
        if($pickupLocations->count() > 0){
            return $pickupLocations;
        }
        if (PickupLocation::where('created_by', $user_id)->exists()) {
            return $pickupLocations;
        }
    ...
    {
        $user_id = getParentSellerId();
        return $this->visibleQuery($user_id)
            ->where('status', 1)
            ->get();
    }
    ...
    {
        return PickupLocation::with(['country', 'state', 'city', 'costCenter'])
            ->where('created_by', $userId)
            ->where(function ($query) {
                $query->whereNull('cost_center_id')
                    ->orWhereHas('costCenter');
            });
    }
    ...
    public function create(array $data)
    {
        ...
        if(isModuleActive('ShipRocket') && $flag){
            $shipRocketRepo = new ShipRocketRepository(); // NOSONAR
    ...
    {
        $location = PickupLocation::with('costCenter')->findOrFail($data['id']);

        // Si intenta activar, verificar que el CC esté activo y no eliminado
        if ($data['status'] == 1) {
            $cc = $location->costCenter;
            if (!$cc || $cc->trashed() || $cc->status == 0) {
                throw new CannotActivatePickupLocationException(__('shipping.cannot_activate_pickup_location'));
            }
        }
    ...
    {
        $user_id = getParentSellerId();
        $location = PickupLocation::with('costCenter')
            ->where('id', $data['id'])
        ...
        if ((int) $location->status !== 1 || $ccBlocked) {
            throw new CannotActivatePickupLocationException(__('shipping.cannot_set_default_pickup_location'));
        }

        DB::table('pickup_locations')->where('created_by',$user_id)->update(array('is_default' => 0));
        return $location->update(['is_default' => $data['set_default']]);
    }
