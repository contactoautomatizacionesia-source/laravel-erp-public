...
class CarrierRepository
{
    ...
    public function paginate($perPage = 10, $path = null)
    {
        $carriers = $this->all()->values();
        $currentPage = LengthAwarePaginator::resolveCurrentPage();
        $currentItems = $carriers->slice(($currentPage - 1) * $perPage, $perPage)->values();
        ...
        return new LengthAwarePaginator(
            ...
            $currentPage,
            [
                'path' => $path ?? route('shipping.carriers.index'),
                'query' => request()->query(),
            ]
        );
    }
    ...
    public function create(array $data)
    {
        ...
        if(isModuleActive('FrontendMultiLang')){
            ...
            $carrier->setTranslation('name','en',$data['name']);
            $carrier->logo = $data['logo'];
            $carrier->tracking_url = $data['tracking_url'];
            $carrier->created_by = $data['created_by'];
            $carrier->save();
    ...
    public function update(array $data,$id)
    {
        ...
        if(isModuleActive('FrontendMultiLang')){
            ...
            $carrier->setTranslation('name','en',$data['name']);
            $carrier->logo = isset($data['logo_url'])?$data['logo_url']:$row->logo;
            $carrier->tracking_url = $data['tracking_url'];
            $carrier->save();
        }
