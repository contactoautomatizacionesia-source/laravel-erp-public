...
class Folder extends Model
{
    ...
    /**
     * Retorna la ruta física en storage basada en IDs de la jerarquía.
     * Al usar IDs en lugar de nombres, la ruta es inmutable:
     * renombrar una carpeta NO mueve ni rompe los archivos ya subidos.
     * Ejemplo: "fileExplorer/1/3/7"
     */
    public function getPhysicalPath(): string
    {
        $segments = $this->getBreadcrumb()
            ->map(fn($folder) => $folder->id)
            ->toArray();

        return 'fileExplorer/' . implode('/', $segments);
    }
