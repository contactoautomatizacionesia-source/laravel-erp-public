                        ...
                        <div class="col-lg-6">
                            <div class="products_view_left text-center mb-35">
                                <div class="products_image mb-25">
                                    <img src="{{ showImage(@$product->productSKU->product->thumbnail_image_source) }}" alt="">
                                </div>
                                ...
                                <div class="products_details_list">
                                    ...
                                    <div class="products_details_single">
                                        ...
                                        @if($product->productSKU)
                                            @if($product->productSKU->product->product_type == 1)
                                                <span>{{__("product.physical_product")}}</span>
                                            @else
                                                <span>{{__("product.digital_product")}}</span>
                                            @endif
                                        @endif
                                    </div>
                                    <div class="products_details_single">
                                    ...
                                    @if(!empty($product->gstGroup))
                                        @php
                                            $cgst = (array) json_decode($product->gstGroup->same_state_gst);
                                            $igst = (array) json_decode($product->gstGroup->outsite_state_gst);
                                        @endphp
                                        <div class="products_details_single">
                                            <span>{{__('gst.gst')}}: </span>
                                            <span> @if(!empty($product->gstGroup->same_state_gst)) {{__('gst.same_state_GST')}}: @foreach($cgst as $key => $c) {{ getGstName($key) }} @php @endphp  {{ $c }}% @endforeach @endif  <br> @if(!empty($product->gstGroup->outsite_state_gst)) {{__('gst.outsite_state_GST')}}: @foreach($igst as $l =>  $i) {{ getGstName($l) }} {{ $i }}% @endforeach  @endif </span>
                                        </div>
                                    @elseif($product->tax != 0)
                                        <div class="products_details_single">
                                            <span>{{__('product.tax')}}: </span>
                                            <span>{{ getNumberTranslate(($product->tax_type == 1) ? single_price($product->tax) : $product->tax. "%") }}</span>
                                        </div>
                                    @endif
                                    <div class="products_details_single">
                                ...
                                <div class="mb-35">
                                    ...
                                    <div class="row">
                                        @foreach ($product->productSKU->product->gallary_images as $key => $gallary_image)
                                            <div class="col-md-3">
                                                <img  height="50px" width="70px" src="{{showImage($gallary_image->images_source)}}" alt="{{$product->product_name}}">
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
