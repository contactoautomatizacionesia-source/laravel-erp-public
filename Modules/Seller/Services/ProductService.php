...
class ProductService
{
    ...
    public function __construct(ProductRepository $productRepository, DoubleApprovalService $doubleApprovalService){
    ...
    public function store($data){
        ...
        if(isset($data['thumbnail_image'])){
            ...
            if($media_img){
                ...
                if ($host == 'Dropbox') {
                    $data['thum_img_src'] = $thumbnail_image['images_source'];
    ...
    public function update($data, $id){
        ...
        // Verificar si product stock está activo en los parámetros
        $approvalParam = ParameterSetting::where('slug', 'product-stock')->first();

        if ($approvalParam && $approvalParam->is_active) {

            $totalStock = array_sum($data['stock']);
            $maxStock = $product->product->max_stock;
            $minStock = $product->product->min_stock;
            // Inicializamos la variable del slug
            $notificationSlug = null;

            if ($totalStock > $maxStock) {
                $notificationSlug = 'overstock_alert';
            } elseif ($totalStock < $minStock) {
                $notificationSlug = 'low_stock_alert';
            }

            // Si se disparó alguna de las dos condiciones, enviamos la notificación
            if ($notificationSlug) {
                $notificationData = [
                    'product_id'    => $product->id,
                    'product_name'  => $data['product_name'],
                    'skus'          => $this->getThisSKUProduct($id),
                    'current_stock' => $totalStock,
                    'min_stock'     => $minStock,
                    'max_stock'     => $maxStock
                ];

                $this->doubleApprovalService->sendStockAlertNotification(
                    $notificationSlug, // Enviamos el slug dinámico
                    $notificationData,
                );
            }
        }
        ...
        if(isset($data['thumbnail_image']) && @$product->thumb_image_media->media_id != $data['thumbnail_image']){
            ...
            if(strpos($product->thum_img, 'amazonaws.com') != false || strpos($product->thum_img, 'digitaloceanspaces.com') != false || strpos($product->thum_img, 'drive.google.com') != false || strpos($product->thum_img, 'wasabisys.com') != false || strpos($product->thum_img, 'backblazeb2.com') != false || strpos($product->thum_img, 'dropboxusercontent.com') != false || strpos($product->thum_img, 'storage.googleapis.com') != false || strpos($product->thum_img, 'contabostorage.com') != false || strpos($product->thum_img, 'b-cdn.net') != false){
    ...
    public function get_seller_product_variant_wise_price($data){
