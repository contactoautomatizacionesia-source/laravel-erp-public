@push('scripts')
    ...
    (function($) {
        ...
        $(document).on('click','.saveBtn',function() {
            ...
            if ($('input[name=product_type]:checked').val() === '2') {
                $('input[name*=selling_price_sku]').each(function() {
                    if ($(this).val()== '') {
                        requireMatch = 1;
                        toastr.warning("{{__('product.please_input_selling_price')}}");
                    }else if ($(this).val()== 0) {
                        requireMatch = 1;
                        toastr.warning("{{__('product.please_input_selling_price_minimum_one')}}");
                    }
                });
            }
            if (requireMatch == 1) {
                ...
                // Si las validaciones básicas pasan, llamamos a la lógica de cambios
                event.preventDefault(); // Detenemos el submit normal para validar cambios vía AJAX
                // LIMPIEZA: Eliminamos cualquier rastro de confirmaciones previas antes de chequear
                $('#choice_form').find('input[name="change_confirmed"]').remove();
                $('#choice_form').find('input[name="change_observation"]').remove();
                checkProductChanges();
            }
        });
        ...
        function weightHeightDivShow(){
            // Ocultamos el overlay (desbloquear)
            $('.physical_blocked_overlay').hide();
            
            // Habilitamos los inputs usando su atributo 'name' y .prop()
            $("input[name='weight']").prop('disabled', false);
            $("input[name='length']").prop('disabled', false);
            $("input[name='breadth']").prop('disabled', false);
            $("input[name='height']").prop('disabled', false);
            $("input[name='additional_shipping']").prop('disabled', false);
        }

        function weightHeightDivHide(){
            // Mostramos el overlay (bloquear)
            $('.physical_blocked_overlay').css('display', 'flex');
            
            // Deshabilitamos los inputs usando su atributo 'name' y .prop()
            $("input[name='weight']").prop('disabled', true);
            $("input[name='length']").prop('disabled', true);
            $("input[name='breadth']").prop('disabled', true);
            $("input[name='height']").prop('disabled', true);
            $("input[name='additional_shipping']").prop('disabled', true);
        }
        $(document).on('change', '.variant_digital_file_change', function(event){
        ...
        var lotStockUrl = "{{ route('product.lot-stock') }}";
        var lotStockLabels = {
            loading: "{{ __('product.lot_stock_loading') }}",
            empty: "{{ __('product.lot_stock_empty') }}",
            error: "{{ __('product.lot_stock_error') }}",
            no_lot: "{{ __('product.lot_stock_no_lot') }}",
            na: "{{ __('product.lot_stock_na') }}"
        };
        ...
        $(document).on('click', '.btn-lot-detail', function () {
            let skuId = $(this).data('sku-id');
            let skuLabel = $(this).data('sku-label') || lotStockLabels.na;
            if (!skuId) {
                return;
            }
            $('#lot_stock_sku_label').text(skuLabel);
            $('#lot_stock_total').text('...');
            $('#lot_stock_tbody').html('<tr><td colspan="5" class="text-center text-muted small">' + lotStockLabels.loading + '</td></tr>');
            $('#lot_stock_empty').addClass('d-none').text(lotStockLabels.empty);
            $('#lot_stock_modal').modal('show');

            $.get(lotStockUrl, { product_sku_id: skuId }, function (resp) {
                let rows = '';
                if (resp && resp.data && resp.data.length) {
                    resp.data.forEach(function (item) {
                        let statusHtml = item.status ? `<span class="${item.status.class}">${item.status.label}</span>` : lotStockLabels.na;
                        let qtyVal = (item.qty !== undefined && item.qty !== null) ? item.qty : 0;
                        rows += `<tr>
                            <td>${item.lot_number || lotStockLabels.no_lot}</td>
                            <td>${item.manufacture_date || lotStockLabels.na}</td>
                            <td>${item.expiration_date || lotStockLabels.na}</td>
                            <td>${statusHtml}</td>
                            <td class="text-right">${qtyVal}</td>
                        </tr>`;
                    });
                    $('#lot_stock_tbody').html(rows);
                    $('#lot_stock_empty').addClass('d-none');
                } else {
                    $('#lot_stock_tbody').html('');
                    $('#lot_stock_empty').removeClass('d-none').text(lotStockLabels.empty);
                }
                let total = (resp && resp.total_qty !== undefined && resp.total_qty !== null) ? resp.total_qty : 0;
                $('#lot_stock_total').text(total);
            }).fail(function () {
                $('#lot_stock_tbody').html('');
                $('#lot_stock_empty').removeClass('d-none').text(lotStockLabels.error);
                $('#lot_stock_total').text('0');
            });
        });
        ...
        function get_combinations(el) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data){applyCurrencyMask(document.querySelector('.sku_combination'));} //aplicamos mascara de precios al cargar la data de las combinaciones 
        ...
        // Limpiar campos de mascara antes de enviar 
        document.getElementById('choice_form').addEventListener('submit', function () {

            this.querySelectorAll('.currency-mask').forEach(input => {
                if (input.value !== '') {
    ...
    (function ($) {
        ...
        /**
         * Función para detectar cambios vía AJAX antes de procesar
         */
        window.checkProductChanges = function() {
            // Si ya fue confirmado previamente, enviamos directamente
            if ($('#change_confirmed').length > 0) {
                $('#choice_form')[0].submit();
                return;
            }

            $("#pre-loader").removeClass('d-none');
            let form = $('#choice_form')[0];
            let formData = new FormData(form);
            formData.append('check_changes_only', 1);
        ...
        /**
         * Ejecución del botón de confirmar en el modal
         */
        $(document).on('click', '#confirm_update_btn', function() {
            const observation = $('#change_observation_input').val();
            // Validación de seguridad por si se saltan la restricción del botón
            if (observation.length < 200) {
                toastr.error("{{ __('common.observation_min_200') }}", "{{ __('common.error') }}");
                
                // Re-deshabilitar el botón por seguridad
                $(this).prop('disabled', true);
                return false;
            }
            // Si pasa la validación, procedemos con el envío final
            submitFinalForm(observation);
        });

        function submitFinalForm(observation = '') {
            $('#changeConfirmationModal').modal('hide');
            $("#pre-loader").removeClass('d-none');
            let form = $('#choice_form');
            // Eliminar inputs previos si existen (evita duplicidad si el usuario reintenta)
            form.find('input[name="change_confirmed"]').remove();
            form.find('input[name="change_observation"]').remove();
            form.append(`<input type="hidden" name="change_confirmed" value="1">`);
            form.append(`<input type="hidden" name="change_observation" value="${observation}">`);
            
            // Importante: Limpiar máscaras de moneda antes de enviar (tu código original lo requiere)
            form.find('.currency-mask').each(function() {
