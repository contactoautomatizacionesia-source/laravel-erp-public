    ...
    (function($) {
        ...
        $(document).ready(function() {
            ...
            $(document).on('click','.saveBtn',function() {
                ...
                $('#error_min_stock').text('');
                $('#error_max_stock').text('');
                var requireMatch = 0;
                ...
                @endif
                if ($('input[name=product_type]:checked').val() === '1' && $("#sku_single").val() === '') {
                    requireMatch = 1;
                ...
                if ($("#min_stock").val() === '') {
                    requireMatch = 1;
                    $('#error_min_stock').text("{{ __('product.please_input_min_stock') }}");
                }
                if ($("#max_stock").val() === '') {
                    requireMatch = 1;
                    $('#error_max_stock').text("{{ __('product.please_input_max_stock') }}");
                }
                if ($("#min_stock").val() !== '' && $("#max_stock").val() !== '') {
                    var minStock = parseInt($("#min_stock").val());
                    var maxStock = parseInt($("#max_stock").val());
                    if (minStock > maxStock) {
                        requireMatch = 1;
                        $('#error_min_stock').text("{{ __('product.min_stock_greater_than_max') }}");
                        $('#error_max_stock').text("{{ __('product.max_stock_less_than_min') }}");
                    }
                }
                ...
                if ($('input[name=product_type]:checked').val() === '2' && $("#choice_options").val()== undefined) {
                    requireMatch = 1;
                ...
                if (requireMatch == 1) {
                    ...
                    var $firstError = $('.text-danger').filter(function() {
