    ...
    <table class="table table-bordered sku_table">
        ...
        <tr>
            ...
            @if(!isModuleActive('MultiVendor'))
                <td width="13%" class="text-center stock_td d-none">
                    <span class="d-block">{{__('product.stock')}}</span>
                </td>
            @endif
            ...
            <td width="20%" class="text-center variant_physical_div">
                <label for="" class="control-label">{{__('common.image')}} ({{getNumberTranslate(165)}} X {{getNumberTranslate(165)}}){{__('common.px')}}</label>
            </td>
            <td width="15%" class="text-center variant_digital_div">
                <label for="" class="control-label">{{__('product.file')}}</label>
            </td>
            <td width="15%" class="variant_digital_div">
                <label for="" class="control-label">{{__('product.in_app_purchase_code') }}</label>
            </td>
        </tr>
        </thead>
        <tbody>
        ...
        @foreach ($combinations as $key => $combination)
            @php
                $sku = '';
                $ttt = $key;
                $sku .= str_replace(' ', '-', $variant_sku_prefix);
                $str = '';
                $str_id = '';
                $attribute_id = '';
            ...
            @if(strlen($str) > 0)
                <tr class="variant text-center">
                    <td class="">
                        ...
                        <label for="" class="control-label " style="color: #000">{{ $str }}</label>
                    </td>
                    <td class="">
                        <input class=" selling_price style_selling_price_field currency-mask" type="text" name="selling_price_sku[]" value="{{ $query_1 ? $query_1->selling_price : 1 }}" class="form-control" required>
                        @if (isModuleActive('WholeSale') && !isModuleActive('MultiVendor'))
                            <button type="button" data-toggle="modal" tabindex="-1"
                                    ...
                                    class="mt-1 add_variant_whole_sale_price" style="background: var(--base_color);color: white;border-radius: 10px;padding: 5px;font-size: 10px">
                                {{__('wholesale.ranges')}}
                            </button>
                        @endif
                    </td>
                    @if(!isModuleActive('MultiVendor'))
                        <td class="text-center stock_td d-none">
                            @php
                                $lotStock = 0;
                                if ($query_1 && isset($lotStockBySku)) {
                                    $lotStock = (float) ($lotStockBySku[$query_1->id] ?? 0);
                                }
                            @endphp
                            <span class="font-weight-bold d-block">{{ $lotStock }}</span>
                            @if($query_1)
                                <button type="button"
                                        class="btn-toolkit btn-secondary-outline btn-xs mt-1 btn-lot-detail p-1"
                                        data-sku-id="{{ $query_1->id }}"
                                        data-sku-label="{{ $query_1->sku ?? $sku }}">
                                    {{ __('product.lot_stock_detail_btn') }}
                                </button>
                            @else
                                <button type="button" class="btn-toolkit btn-secondary-outline btn-xs mt-1 p-1" disabled>
                                    {{ __('product.lot_stock_detail_btn') }}
                                </button>
                            @endif
                        </td>
                    @endif
                    <td class="">
                        <input class="primary_input_field" type="text" name="sku[]"
                               value="{{ ($query_1) ? @$query_1->sku : $sku }}" class="form-control">
                        <input type="hidden" name="track_sku[]" value="{{$sku}}">
                    </td>
                    <td class="variant_physical_div text-center ">
                        <div class="primary_input">
                            <div class="primary_file_uploader " data-toggle="amazuploader" data-multiple="false"
                    ...
                    <td class="variant_digital_div text-center">
                        <div class="primary_input">
                            ...
                            <div class="primary_file_uploader">
                                <input class="primary-input " type="text" id="variant_digital_file_{{$valIncre}}"
                    ...
                    <td class="variant_digital_div text-center">
                        <input class="primary_input_field mt-1" type="text" name="var_in_app_purchase_code[]"
                                   value="{{ ($query_1) ? @$query_1->in_app_purchase : "" }}"  class="form-control">
                    </td>
                </tr>
            @endif
            @if(isModuleActive('WholeSale'))
