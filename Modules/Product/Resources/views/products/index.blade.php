...
@section('page-title', __('product.product_list'))
@section('styles')
                    ...
                    <div class="box_header_right">
                        <div class=" pos_tab_btn justify-content-end">
                            <ul class="nav ign-scrollbar flex-nowrap w-100 overflow-auto pb-2">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link action" href="{{route("product.create")}}"><i class="ti-plus mr-2"></i>{{__('product.add_new_product')}}</a>
                                </li>
                                ...
                                @if (permissionCheck('product.get-data'))
                                    <li class="nav-item">
                                        <a class="nav-link active show" href="#order_processing_data" role="tab"
                                            data-toggle="tab" id="product_list_id" aria-selected="true">{{__('product.products')}}</a>
                                    </li>
                                @endif
                                ...
                                @endif
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link" href="#reported_product_data" role="tab" data-toggle="tab" id="reported_products"
                                            aria-selected="true">{{__('product.reported')}}</a>
                                    </li>
                            </ul>
                        </div>
                    </div>
...
<script type="text/javascript">
    (function($){
        ...
        $(document).ready(function(){
            const hasStock = (module_check === 'false');
            const indexCol = { data: 'DT_RowIndex', name: 'id', render:function(data){ return numbertrans(data) } };
            ...
            if (hasStock) {
                columnDataFull.push({ data: 'stock', name: 'stock' });
            }

            columnDataFull.push(
                { data: 'unit_price', name: 'unit_price' },
                { data: 'unit_points', name: 'unit_points' },
            ...
            if (hasStock) {
                columnDataSimple.push({ data: 'stock', name: 'stock' });
            }

            columnDataSimple.push(
                { data: 'status', name: 'status' },
                { data: 'action', name: 'action', searchable:false, orderable:false }
            );

            const columnDataRequest = [
                indexCol,
                { data: 'product_name', name: 'product_name' },
                { data: 'product_type', name: 'product_type' },
                { data: 'brand', name: 'brand', searchable:false, orderable:false },
        ...
        function mainProductDataTable(){
            if (!$('#mainProductTable').length) return;
            initGlobalDataTable('#mainProductTable', "{{route('product.get-data')}}", columnDataFull, {
                columnDefs: [{ visible: true, targets: [-1], responsivePriority: 1 }]
            });
        }
        function draftedProductTable(){
            if (!$('#drafted_product_table').length) return;
            initGlobalDataTable('#drafted_product_table', "{{route('product.get-data')}}" + '?table=drafted', columnDataSimple);
        }
        function disabledProductDataTable(){
            if (!$('#disabledProductTable').length) return;
            initGlobalDataTable('#disabledProductTable', "{{route('product.get-data')}}" + '?table=disable', columnDataSimple);
        }
        function alertProductDataTable(){
            if (!$('#alertProductTable').length) return;
            initGlobalDataTable('#alertProductTable', "{{route('product.get-data')}}" + '?table=alert', columnDataSimple);
        }
        function stockoutProductDataTable(){
            if (!$('#stockoutProductTable').length) return;
            initGlobalDataTable('#stockoutProductTable', "{{route('product.get-data')}}" + '?table=stockout', columnDataSimple);
        }

        function requestProductDataTable(){
            if (!$('#requestProductTable').length) return;
            initGlobalDataTable('#requestProductTable', "{{route('product.request-get-data')}}", [
                { data: 'DT_RowIndex', name: 'id', render: function(data){ return numbertrans(data); } },
                { data: 'product_name', name: 'product_name' },
                { data: 'product_type', name: 'product_type' },
        ...
        function SKUDataTable(){
            if (!$('#SKUTable').length) return;
            initGlobalDataTable('#SKUTable', "{{route('product.get-data-sku')}}", [
                { data: 'DT_RowIndex', name: 'id', render: function(data){ return numbertrans(data); } },
        ...
        function reportedProducts(){
            if (!$('#reported_product_datatable').length) return;
            initGlobalDataTable('#reported_product_datatable', "{{route('product.reportedProducts')}}", [
                { data: 'DT_RowIndex', name: 'id', render: function(data){ return numbertrans(data); } },
                { data: 'product_name', name: 'product_name' },
                { data: 'logo', name: 'logo' },
                { data: 'reason', name: 'reason' },
