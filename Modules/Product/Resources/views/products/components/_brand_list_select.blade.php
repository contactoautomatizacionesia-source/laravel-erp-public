<div class="primary_input mb-15">
    ...
    <select  class="brand_id mb-15 primary_select2" id="brand_id" name="brand_id">
        ...
        @if(old('brand_id'))
        @php
            $old_selected_brand = \Modules\Product\Entities\Brand::find(old('brand_id'));
        @endphp
        @if($old_selected_brand)
        <option value="{{ $old_selected_brand->id }}" selected>{{ $old_selected_brand->getTranslation('name', app()->getLocale()) }}</option>
        @endif
