...
<div class="primary_input mb-25">
    ...
    <select name="category_ids[]" id="category_id" class="mb-15 category_id" @if(app('general_setting')->multi_category == 1) multiple @elseif(isset($product) && count($product->categories) > 1) multiple @endif required="1">
        ...
        @if(old('category_ids'))
            @foreach(\Modules\Product\Entities\Category::whereIn('id', old('category_ids'))->get() as $category)
                <option value="{{ $category->id }}" selected>{{ $category->getTranslation('name', app()->getLocale()) }}</option>
            @endforeach
    ...
    <span class="text-danger" id="error_category_ids">{{ $errors->first('category_ids') }}</span>
</div>
