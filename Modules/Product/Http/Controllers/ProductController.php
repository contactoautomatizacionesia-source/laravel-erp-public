...
class ProductController extends Controller
{
    ...
    public function getData()
    {
        ...
        return DataTables::of($products)
            ...
            ->addColumn('sku', function ($products) {
                return @$products->skus->first()->sku ?? '';
            })
            ->addColumn('category_list', function ($products) {
                return $products->categories->pluck('name')->implode(', ');
            })
            ->addColumn('brand', function ($products) {
                return @$products->brand->name ?? '';
            })
            ->addColumn('prefix', function ($products) {
                return $products->variant_sku_prefix ?? '';
            })
            ->addColumn('logo', function ($products) {
                return view('product::products.components._product_logo_td', compact('products'));
            })
            ->addColumn('min_stock', function ($products) {
                return @$products->min_stock ?? 1;
            })
            ->addColumn('max_stock', function ($products) {
                return @$products->max_stock ?? 1;
            })
            ->addColumn('unit_price', function ($products) {
                return single_price(@$products->skus->first()->selling_price ?? 0);
            })
            ->addColumn('unit_points', function ($products) {
                return $products->club_point ?? 0;
            })
            ->addColumn('created_by_user', function ($products) {
    ...
    public function show(Request $request)
    {
        ...
        $mainStockBySkuId = \Modules\CostCenter\Entities\CostCenterInventoryLot::where('location_type', 'main')
        ...
        return view('product::products.product_detail', compact('product', 'mainStockBySkuId'));
    }
    public function showSimple(Request $request)
    {
        $data['product'] = $this->productService->findById($request->id);
        return view('product::products.components._product_simple_detail', $data);
    }

    public function lotStock(Request $request)
    {
        $validated = $request->validate([
            'product_sku_id' => 'required|integer|exists:product_sku,id',
        ]);

        $skuId = (int) $validated['product_sku_id'];
        $lots = CostCenterInventoryLot::with('lot')
        ...
        $service = app(InventoryEntryService::class);
        $mapped = $lots->map(function ($row) use ($service) {
            $lot = $row->lot;
            $badge = $lot ? $service->getLotStatusBadge($lot) : null;

            return [
                'lot_number' => $lot?->lot_number ?? 'Sin lote',
                'manufacture_date' => $lot?->manufacture_date?->format('Y-m-d'),
                'expiration_date' => $lot?->expiration_date?->format('Y-m-d'),
                'qty' => (float) $row->qty,
                'status' => $badge ? [
                    'label' => $badge['label'] ?? '',
                    'class' => $badge['class'] ?? 'badge_1',
                ] : null,
            ];
        });
    ...
    {
        // Si hay cambios retornar para mostrar modal
        if ($request->has('check_changes_only')) {
            $productBefore = $this->productService->findById($id);
            return response()->json([
                'requires_confirmation' => $this->hasProductChanges($productBefore, $request->all())
            ]);
        }
        ...
        try {
            ...
            // Si el cambio fue confirmado, validamos la observación en el Backend
            if ($request->has('change_confirmed') === "1") {
                $request->validate([
                    'change_observation' => 'required|string|min:200'
                ], [
                    // Mensaje personalizado para depurar
                    'change_observation.required' => 'La observación es obligatoria para guardar los cambios.',
                    'change_observation.min' => 'La observación debe tener al menos 200 caracteres.'
                ]);
            }

           // Llamamos al servicio (que procesa imágenes/pdf y luego llama al repository)
    ...
    public function sku_combination_edit(Request $request)
    {
        ...
        $attribute = $request->choice_no;
        $combinations = combinations($options);
        $skuIds = $product->skus->pluck('id')->filter()->values();
        $lotStockBySku = collect();
        if ($skuIds->isNotEmpty()) {
            $lotStockBySku = CostCenterInventoryLot::where('location_type', 'main')
                ->whereNull('location_id')
                ->whereIn('product_sku_id', $skuIds)
        ...
        return view('product::products.sku_combinations_edit', compact('combinations', 'variant_sku_prefix', 'product', 'attribute', 'lotStockBySku'));
    ...
    {
        $products = $this->productService->getActiveProductsForSelect($request->q);
        return response()->json($products);
    }

    /**
     * Identifica si existen cambios entre los datos actuales y los nuevos.
     *
     * @param \Modules\Product\Entities\Product $product
     * @param array $newData
     * @return bool True si hay cambios, False de lo contrario.
     */
    private function hasProductChanges($product, array $newData): bool
    {
        // Sincronizamos los datos al modelo sin persistir en la DB
        $product->fill($newData);

        // getDirty() devuelve un array con los campos que cambiaron.
        // Retornamos true si el array no está vacío.
        $changes = $product->getDirty();

        $ignoredFields = [
            'updated_at',
            'slug',
            // 'meta_image', // Ignoramos este ya que se procesa por Media Manager
        ...
        // Limpieza de falsos positivos (Comparación débil)
        // Esto evita que cambios de string a integer disparen la modal
