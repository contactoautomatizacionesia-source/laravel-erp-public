...
class BrandController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->input('keyword') != null) {
            $data['brands'] = $this->brandService->dashboardSearch($request->input('keyword'));
    ...
    public function destroy($id)
    {
        try {
            ...
            // Evaluamos la respuesta específica de los productos
            if ($result == "has_products") {
                Toastr::warning(__('product.brand_has_associated_products'), __('common.warning'));
            } elseif ($result == "not_possible") {
                Toastr::warning(__('product.related_data_exist_in_multiple_directory'), __('common.warning'));
            } else {
