...
class ProductRepository
{
    ...
    public function getProduct()
    {
        ...
        return  Product::with(['brand','unit_type', 'categories', 'skus', 'seller'])->select($get_columns)->where('is_approved', 1);
    }
    ...
    public function create(array $data)
    {
        ...
        if (app('gst_config')['enable_gst'] == "only_tax" && app('general_setting')->price_with_vat) {
            if ($data['product_type'] == 2) {
                $selling_price = selling_price($data['selling_price_sku'][0],$data['discount_type'],$data['discount']);
            }else {
                $selling_price = selling_price($data['selling_price'],$data['discount_type'],$data['discount']);
            }
            $gst = GstTax::find($data['tax_id']);
        ...
        unset($data['gst_group']);

        $in_app_purchase_code = isset($data['in_app_purchase_code']) ? $data['in_app_purchase_code']:'';
        unset($data['in_app_purchase_code']);
       ...
       // send notification from seller request
            ...
            foreach ($data['track_sku'] as $key => $variant_sku) {
                ...
                if (!isModuleActive('MultiVendor')) {
                    if ($data['stock_manage'] == 1) {
                        $stock = isset($data['sku_stock'][$key]) ? $data['sku_stock'][$key] : 0;
                    }
                }
                $product_sku->product_stock = $stock;
        ...
        if (auth()->user()->role->type == 'superadmin' || auth()->user()->role->type == 'admin' || auth()->user()->role->type == 'staff') {
            ...
            $sellerProduct->subtitle_1 = $data['subtitle_1'] ?? null;
            $sellerProduct->subtitle_2 = $data['subtitle_2'] ?? null;
            $sellerProduct->save();
    ...
    public function update(array $data, $id)
    {
        ...
        // Identificar cambios ANTES de actualizar
        $detailedChanges = $this->identifyChanges($product, $data);
        ...
        $in_app_purchase_code = isset($data['in_app_purchase_code']) ? $data['in_app_purchase_code']:'';
        unset($data['in_app_purchase_code']);
        $data['is_physical'] = isset($data['is_physical']) ? $data['is_physical']:1;
        $subtitle_1 = $data['subtitle_1'] ?? null;
        $subtitle_2 = $data['subtitle_2'] ?? null;
        unset($data['subtitle_1'], $data['subtitle_2']);
        $product->update($data);
        if (!isModuleActive('FrontendMultiLang')) {
            $locale = app()->getLocale();
            $product->setTranslation('subtitle_1', $locale, $subtitle_1 ?? null);
            $product->setTranslation('subtitle_2', $locale, $subtitle_2 ?? null);
            $product->save();
        }
        if (isModuleActive('FrontendMultiLang')) {
            if (!isModuleActive('MultiVendor')) {
                ...
                $productName = [];
                $product_name = json_decode($dbproduct->product_name);
                foreach ($product_name as $key => $value) {
                    $productName[$key] = $value;
                }
                $productSubtitle_1 = [];
                $product_subtitle_1 = json_decode($dbproduct->subtitle_1);
                foreach ($product_subtitle_1 as $key => $value) {
                    $productSubtitle_1[$key] = $value;
                }
                $productSubtitle_2 = [];
                $product_subtitle_2 = json_decode($dbproduct->subtitle_2);
                foreach ($product_subtitle_2 as $key => $value) {
                    $productSubtitle_2[$key] = $value;
                }
                ...
                $sellerProduct->discount = $product->discount;
                $sellerProduct->discount_type = $product->discount_type;
                $sellerProduct->slug = $productSlug;
        ...
        if ($product->product_type == 1) {
            ...
            if ($data['is_physical'] == 0 && !empty($data['single_digital_file'])) {
                $name = uniqid() . $data['single_digital_file']->getClientOriginalName();
                $data['single_digital_file']->move(public_path() . '/uploads/digital_file/', $name);
                $data['file_source'] = '/uploads/digital_file/' . $name;
                $old_file = DigitalFile::where('product_sku_id', $product_sku->id)->first();
                if($old_file){
                    if (File::exists(asset_path($old_file->file_source))) {
                        File::delete(asset_path($old_file->file_source));
                    }
                    $old_file->update([
                        'file_source' => $data['file_source']
                    ]);
            ...
            if(product_attribute_editable($product->id) === false){
                foreach ($data['track_sku'] as $key => $variant_sku) {
                    ...
                    if ($sku_exist == null) {
                        ...
                        $old_sku_ids =  $product->skus->pluck('id');
                ...
                foreach($old_skus as $sku){
                    ...
                    if($sku->digital_file){
                        $old_digital_files[] = $sku->digital_file;
                    }
                    $old_sku_ids[] = $sku->id;
                }
                ProductSku::destroy($old_sku_ids);
                ...
                foreach ($data['track_sku'] as $key => $variant_sku) {
                    ...
                    if ($data['is_physical'] == 0 && !empty($data['digital_file']) && isset($data['digital_file'][$key])) {
                        ...
                        if(isset($old_digital_files[$key])){
                            $old_digital_files[$key]->delete();
                            if (File::exists(asset_path($old_digital_files[$key]->file_source))) {
                                File::delete(asset_path($old_digital_files[$key]->file_source));
                            }
                        }
                    }elseif(isset($old_digital_files[$key])){
                        $old_file = $old_digital_files[$key];
                        $old_file->update([
                            'product_sku_id' => $product_sku->id
                        ]);
                    }
        ...
        if(auth()->user()->role->type == 'seller' && isModuleActive('MultiVendor')){
            ...
            $notification = NotificationSetting::where('slug','seller-product-update')->first();
            if ($notification) {
                $this->notificationSend($notification->id, $product->created_by);
            }
        }

         // --- Lógica de Alerta de cambios en el producto ---
        if (isset($data['change_confirmed'])) {

            $product->refresh(); // Asegura que sean los valores recién guardados
            $notificationSlug = 'alert_product_update';
            $notificationData = [];
            
            // CREAMOS EL OBJETO DE DATOS DEL PRODUCTO PARA LA NOTIFICACIÓN
            $notificationData = [
                'product_id'    => $product->id,
                'product_name'  => $product->product_name,
                'skus'          => $product->skus->pluck('sku')->toArray(),
                'observation'   => $data['change_observation'] ?? '', // Justificación del modal
                'changes'       => $detailedChanges, // Lista detallada
                'updated_by'    => auth()->id(),
            ];
    ...
    private function identifyChanges($product, array $data): array
    {
        $changes = [];

        // Sincronizamos los datos al modelo. Eloquent detecta qué cambió exactamente.
        $product->fill($data);
        $dirtyAttributes = $product->getDirty();

        foreach ($dirtyAttributes as $field => $newValue) {
            // Ignoramos campos técnicos que no aportan valor al administrador
            if (in_array($field, ['updated_at', 'created_at', 'slug'])) {
                continue;
            }

            // Generamos un label dinámico:
            // Buscamos si existe una traducción en common o product
            // Si no, convertimos snake_case a texto legible (Capitalize)
            $label = $this->getDynamicLabel($field);
    ...
    /**
     * Genera una etiqueta legible para el campo de forma dinámica.
     */
    private function getDynamicLabel($field)
    {
        // Intentamos buscar en los archivos de traducción comunes
        $translationKey = "product.{$field}";
        ...
        // Si no hay traducción, transformamos 'variant_sku_prefix' a 'Variant sku prefix'
        ...
        if (count($product->sellerProducts) > 0) {
            if (count($product->sellerProducts) < 2 && $product->sellerProducts->first()->seller->role->type == 'superadmin') {
                if (
                    ...
                    UsedMedia::where('usable_id', $product->id)->where('usable_type', get_class($product))->where('used_for', 'meta_image')->delete();
                    if (File::exists(asset_path($product->pdf))) {
                        File::delete(asset_path($product->pdf));
                    }
                    ...
                    $product_skus = ProductSku::where("product_id", $id)->get();
                    foreach ($product_skus as $sku) {
                        if (!empty($sku->digital_file)) {
                            if (File::exists(asset_path($sku->digital_file->file_source))) {
                                File::delete(asset_path($sku->digital_file->file_source));
                            }
                            $sku->digital_file->delete();
                        }
                        UsedMedia::where('usable_id', $sku->id)->where('usable_type', get_class($sku))->where('used_for', 'variant_image')->delete();
                        $sku->delete();
                    }
    ...
    public function productApproved($data){
        if (isModuleActive('FrontendMultiLang')) {
            ...
            $productSubtitle_1 = [];
            $product_subtitle_1 = json_decode($product->subtitle_1);
            foreach ($product_subtitle_1 as $key => $value) {
                $productSubtitle_1[$key] = $value;
            }
            $productSubtitle_2 = [];
            $product_subtitle_2 = json_decode($product->subtitle_2);
            foreach ($product_subtitle_2 as $key => $value) {
                $productSubtitle_2[$key] = $value;
            }
            $lang = auth()->user()->lang_code;
            ...
            $productSlug = $this->productSlug($product->product_name);
        ...
        $sellerProduct->product_id = $product->id;
        $sellerProduct->product_name = $productName;
        $sellerProduct->stock_manage = $product->stock_manage;
        $sellerProduct->tax = $product->tax;
        $sellerProduct->user_id = $product->created_by;
        $sellerProduct->tax_type = $product->tax_type;
        $sellerProduct->discount =$product->discount;
        $sellerProduct->discount_type = $product->discount_type;
        $sellerProduct->is_approved = 1;
        ...
        foreach ($product_skus as $item) {
            ...
            SellerProductSKU::create([
                ...
                'selling_price' => $item->selling_price,
                'status' => 1,
                'in_app_purchase' => $item->in_app_purchase,
        ...
        $this->routeCheck = 'product.index';
        $this->typeId = EmailTemplateType::where('type', 'product_approve_email_template')->first()->id;
        $notification = NotificationSetting::where('slug','seller-product-approval')->first();
        if ($notification) {
            $this->notificationSend($notification->id, $product->created_by);
        }
        // Send Notification For Followers
        $followers = FollowSeller::where('seller_id', $product->created_by)->get();
        foreach($followers as $follower){
            $notification = NotificationSetting::where('slug','seller-product-create')->first();
            if ($notification) {
                $this->notificationSend($notification->id, $follower->customer_id);
            }
        }
    ...
    {
       $productsku = ProductSku::find($data['id']);
       $productsku->fill($data)->save();
       return $productsku;
    }
    ...
    public function allProducts()
    {
        $user = Auth::user();
        if ($user->role->type == 'superadmin' || $user->role->type == 'admin' || $user->role->type == 'staff') {
            return Product::where('is_approved', 1)->get();
        } else {
            $seller_id = getParentSellerId();
            return Product::where('created_by', $seller_id)->get();
        }
    }

    public function findSku($id, $seller_id)
    {
        return SellerProductSKU::with(['products','mainProduct'])->where('status', 1)->where(['user_id' => $seller_id, 'id'=> $id])->first();
    }
    ...
    {
        return ProductReport::with(['product','user','reason'])->latest();
    }

    public function searchActiveProducts($search = null)
    {
        $query = Product::query();
        ...
        $products = $query->select('id', 'product_name')
                          ->limit(20)
                          ->get();
