...
class BrandRepository
{
    ...
    public function delete($id)
    {
        ...
        // 1. Validación específica para productos
        if (count($brand->products) > 0) {
            return "has_products";
        }
        // 2. Validación general para el resto de relaciones
        elseif (count($brand->MenuElements) > 0 || count($brand->MenuBottomPanel) > 0 || count($brand->Silders) > 0 || count($brand->homepageCustomBrands) > 0) {
            return "not_possible";
        }
        // 3. Proceso de eliminación si está limpio
        else {
