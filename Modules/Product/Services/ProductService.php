...
class ProductService
{
    ...
    public function create($data)
    {
        ...
        if (!empty($data['pdf_file'])) {
            ...
            $data['pdf'] = '/uploads/product_pdf/' . $name;
        }
        if($data['is_physical'] == 0 && $data['product_type'] == 2) {
    ...
    public function updateSkuByID($data){
        ...
        if(isset($data['variant_image'])){
            ...
            $media_img = MediaManager::find($data['variant_image']);
            if($media_img->storage == 'local'){
                $file = asset_path($media_img->file_name);
            }else{
                $file = $media_img->file_name;
            }
            $variant_image = ImageStore::saveImage($file,300, 300);
            if ($host == 'Dropbox') {
                $data['variant_image'] = $variant_image['images_source'];
                $data['file_dropbox'] = $variant_image['file_dropbox'];
            }else{
    ...
    {
        return $this->productRepository->getReportedProduct();
    }

    public function getActiveProductsForSelect($search = null)
    {
        return $this->productRepository->searchActiveProducts($search);
    }
}
