...
class GiftCardRepository
{
    ...
    public function store($data)
    {
        if ($data['product_type'] == 1) {
            ...
            $card = GiftCard::create([
                ...
                'shipping_id' => 1,
                'in_app_purchase' => isset($data['reedem_in_app_purchase_code']) ?  $data['reedem_in_app_purchase_code']:''
            ]);
            $tags = [];
            ...
            foreach ($data['section'] as $v_gift_card){
                ...
                $sections = AddGiftCard::create([
                    ...
                    'in_app_purchase' => gv($v_gift_card, 'in_app_purchase_code',null)
                ]);
    ...
    public function digitalCardUpdate($data, $id){
        ...
        foreach ($data['section'] as $v_gift_card){
            $sections = AddGiftCard::create([
                ...
                'start_date' => Carbon::now()->format('Y-m-d'),
                'in_app_purchase' =>  gv($v_gift_card, 'in_app_purchase_code',null),
               // 'end_date' => (gv($v_gift_card, 'gift_expire_date')) ? Carbon::createFromFormat('m-d-Y', gv($v_gift_card, 'gift_expire_date',null))->format('Y-m-d') : Carbon::now()->format('Y-m-d'),
                'number_of_gift_card' => (integer)gv($v_gift_card, 'number_of_gift_card',null),
            ]);
            if(gv($v_gift_card, 'upload_img_file')){
                Excel::import(new GiftProductImport($sections->id), gv($v_gift_card, 'upload_img_file')->store('temp'));
    ...
    public function send_code_to_mail($data)
    {
        ...
        $secret_code = Str::upper(Str::random(8)).'-'.Str::upper(Str::random(8)).'-'.Str::upper(Str::random(8));
        try {
