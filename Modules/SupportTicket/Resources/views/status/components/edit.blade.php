                    ...
                    <div class="row">
                                    ...
                                    @foreach ($LanguageList as $key => $language)
                                        <div role="tabpanel" class="tab-pane fade @if (auth()->user()->lang_code == $language->code) show active @endif" id="steelement{{$language->code}}">
                                            <div class="primary_input mb-25">
                                                ...
                                                <input class="primary_input_field name" type="text" id="name{{auth()->user()->lang_code == $language->code?$language->code:''}}" name="name[{{$language->code}}]" autocomplete="off"  placeholder="{{__('common.name')}}" value="{{isset($status)?$status->getTranslation('name',$language->code):old('name.'.$language->code)}}">
                        ...
                        @else
                            <div class="col-lg-12">
                                <div class="primary_input mb-25">
                                    <label class="primary_input_label" for="name">{{ __('common.name') }} <span class="text-danger">*</span></label>
                                    <input class="primary_input_field" type="text" id="name" name="name" autocomplete="off" value="{{$status->name}}" placeholder="{{ __('common.name') }}">
                                ...
                                <ul id="theme_nav" class="permission_list sms_list ">
                                    <li>
                                        <label data-id="bg_option"
                                            ...
                                            <input name="status" id="status_active" value="1" {{$status->status == 1?'checked':''}} class="active" type="radio">
                                    ...
                                    <li>
                                        <label data-id="color_option"
                                               class="primary_checkbox d-flex mr-12">
                                            <input name="status" value="0" id="status_inactive" {{$status->status == 0?'checked':''}} class="de_active"
                                                   type="radio">
                                            <span class="checkmark"></span>
                                        </label>
