...
<section class="admin-visitor-area up_st_admin_visitor ign-media-manager">
    <div class="container-fluid p-0">
        <div class="row justify-content-center">
            <div class="col-12">
                <div class="box_header common_table_header">
                    <div class="main-title d-md-flex">
                        <h3 class="mb-0 mr-30 mb_xs_15px mb_sm_20px">{{ __('common.all_uploaded_files') }}</h3>
                    </div>
                    @if(permissionCheck('media-manager.new-upload'))
                    <ul class="d-flex">
                        <li><a class="btn-toolkit btn-primary btn-icon" href="{{url('/media-manager/new-upload')}}"><i class="ti-upload"></i>{{__('common.uploads_new_files')}}</a></li>
                    </ul>
                    @endif
                </div>
            </div>
            <div class="col-lg-12">
                <div class="media_box box_shadow_white p-0">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-lg-12">
                                <form action="" method="GET" class="d-flex align-items-center gap_20 flex-wrap main-title">
                                    <h3 class="flex-fill m-0">{{ __('common.all_files') }}</h3>
                                    <div class="media_header_inputs flex-fill">
                                        <div class="primary_input ">
                                            ...
                                            <div class="primary_input">
                                                <input class="primary_input_field" name="search" placeholder="{{__('common.search')}}" type="text" value="{{isset($_GET['search'])?$_GET['search']:''}}">
                                            </div>
                                    </div>
                                    <button class="btn-toolkit btn-secondary-outline btn-icon"><i class="ti-check"></i>{{__('common.search')}} </button>
                            </form>
                            </div>
                            <div class="col-lg-12">
                                <button class="btn-toolkit btn-icon btn-secondary-outline" id="bulk_select"><i class="ti-check"></i>{{__('common.bulk_select')}} </button>
                                <form action="{{route('media-manager.bulk_delete')}}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <button type="submit" class="btn-toolkit btn-icon btn-danger d-none" id="bulk_delete"><i class="ti-trash"></i>{{__("common.bulk_delete")}}</button>
                            </div>
                        </div>
                                    ...
                                    <div class="amazcart_file_box">
                                        <div class="form-check d-none bulk_delete_checkbox float-right">
                                            <label class="primary_checkbox d-flex" >
                                                <input type="checkbox" name="bulk_select[]" class="attr_checkbox" value="{{$file->id}}">
                                                <span class="checkmark mr_10"></span>
                                                <span class="hidden">Seleccionar medio</span>
                                            </label>
                                        </div>
