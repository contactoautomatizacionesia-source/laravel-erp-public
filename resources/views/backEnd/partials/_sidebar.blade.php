...
@endif
<nav id="sidebar" class="sidebar ign-admin-sidebar">
    ...
    @php
        ...
        $sidebars = \Modules\SidebarManager\Entities\BackendmenuUser::with('children', 'backendMenu')
        ->whereNull('parent_id')->where('user_id', auth()->id())->orderBy('position')->get();
    ...
    @if($sidebars->count())
        <ul id="sidebar_menu" class="px-2">
            ...
            @foreach($sidebars as $key => $section)
                ...
                @php
                    $visibleChildren = $section->children->filter(function ($menu) {
                        return (
                            (!@$menu->backendMenu->module || isModuleActive(@$menu->backendMenu->module)) &&
                            permissionCheck(@$menu->backendMenu->route)
                        );
                    });
                @endphp

                @if($visibleChildren->count() > 0)
                    <span class="menu_seperator text-color-stats">
                    ...
                    @foreach($section->children as $menu)
                        ...
                        @if(!@$menu->backendMenu->module or isModuleActive(@$menu->backendMenu->module))
                            ...
                            @elseif(permissionCheck(@$menu->backendMenu->route))
                                <li class="{{spn_active_link(childrenRoute($menu))}} @if($menu->children->count()) has-sub-menu @endif">
                                    <a href="
