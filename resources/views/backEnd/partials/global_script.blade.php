<script>
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.cart_details_submenu == 'out_of_stock'){
                    toastr.error('No hay más productos disponibles.');
    ...
    function dynamicSelect2WithAjax(selector,url, urlType,dependElement = null){
        let dependValue = '';
        if (dependElement) {
            dependValue = $(dependElement).val();
        }
        $(selector).select2({
            ajax: {
                url: url,
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (data[i].type === "image") {
                            if(data[i].storage == 'local'){
                                ...
                                data[i].file_name +
                                '" class="">';
                            }else if(data[i].storage == 'google'){
                             let query = getQueryParams(data[i].file_name);
                             let image = "https://lh3.google.com/u/0/d/"+query.id;
                             thumb = `<img id="ThumbnailImg" src="${image}" alt="">`;
                            }else{
                                thumb =
