...
<script>
    ...
    function initGlobalDataTable(selector, ajaxUrl = null, columnsConfig = null, additionalOptions = {}) {
        ...
        let defaultOptions = {
            ...
            language: {
                ...
                processing: '<div class="dt-custom-loader"></div>',
                // DataTables Buttons translations (must be inside language.buttons)
                buttons: {
            ...
            buttons: [
                { extend: 'copyHtml5', text: '<i class="fa fa-files-o"></i>', titleAttr: trans('common.copy'), exportOptions: { columns: ':not(:last-child)' } },
                { extend: 'excelHtml5', text: '<i class="fa fa-file-excel-o"></i>', titleAttr: trans('common.excel'), exportOptions: { columns: ':not(:last-child)' } },
                { extend: 'csvHtml5', text: '<i class="fa fa-file-text-o"></i>', titleAttr: trans('common.csv'), exportOptions: { columns: ':not(:last-child)' }, customize: function(csv) { return '\uFEFF' + csv; } },
                { extend: 'pdfHtml5', text: '<i class="fa fa-file-pdf-o"></i>', titleAttr: trans('common.pdf'), orientation: 'landscape', exportOptions: { columns: ':not(:last-child)' }, pageSize: 'A4', margin: [0, 0, 0, 0] },
                { extend: 'print', text: '<i class="fa fa-print"></i>', titleAttr: trans('common.print'), exportOptions: { columns: ':not(:last-child)' } },
                { extend: 'colvis', text: '<i class="fa fa-columns"></i>', titleAttr: trans('common.columns'), postfixButtons: ['colvisRestore'] }
            ],
            columnDefs: [{
                targets: [-1],
        ...
        if (ajaxUrl) {
            ...
            defaultOptions.ajax = { url: ajaxUrl };
        } else {
            defaultOptions.serverSide = false;
        }

        if (columnsConfig) {
            defaultOptions.columns = columnsConfig;
        }

        if (additionalOptions.columnDefs) {
            defaultOptions.columnDefs = additionalOptions.columnDefs;
            delete additionalOptions.columnDefs;
        }
        if (Array.isArray(defaultOptions.buttons) && (additionalOptions.exportTitle || additionalOptions.exportFilename)) {
            defaultOptions.buttons = defaultOptions.buttons.map(btn => {
                if (btn.extend && btn.extend !== 'colvis') {
                    return {
                        ...btn,
                        ...(additionalOptions.exportTitle && { title: additionalOptions.exportTitle }),
                        ...(additionalOptions.exportFilename && { filename: additionalOptions.exportFilename })
                    };
                }
                return btn;
            });
        ...
        delete additionalOptions.exportTitle;
        delete additionalOptions.exportFilename;

        let finalOptions = $.extend(true, {}, defaultOptions, additionalOptions);
    ...
    window.currency_format = function(amount){
        if(_user_currency.length !== 0){
            ...
            let formatedMoney = formatMoney(parseFloat(amount) * parseFloat(user_currency('convert_rate')),config('decimal_limit'),decimal,thousands);
        if(config('currency_symbol_position') === 'left'){
            ...
            let generalformatedMoney =formatMoney(parseFloat(amount),config('decimal_limit'),decimal,thousands);
        if(config('currency_symbol_position') === 'left'){
...
<script src="{{asset(asset_path('backend/js/patch-nice-select.js'))}}"></script>
...
<!-- laraberg -->
{{-- Archivoa movido a el componenete que lo necesita para no cargarlo siempre --}}
{{-- 'backend/vendors/laraberg/js/react.production.min.js' --}}
{{-- 'backend/vendors/laraberg/js/react-dom.production.min.js' --}}
{{-- 'backend/vendors/laraberg/js/laraberg.js' --}}
