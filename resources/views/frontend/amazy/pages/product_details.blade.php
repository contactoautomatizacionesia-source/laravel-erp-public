...
@push('styles')
    ...
    <style>
        ...
        /* Tarjeta PLATINO PREMIUM para vista de detalles */
        .platino-premium-block {
            background: linear-gradient(135deg, #fdfdfd 0%, #ffffff 100%);
            border: 1px solid var(--base_color);
            border-radius: 8px;
            padding: 8px 16px;
            box-shadow: 0 4px 12px rgb(241, 241, 241);
            display: flex;
        ...
        .product_contect_card, .amazcart_delivery_wiz {
            background-color: #f4f7f9;
            box-shadow: 0px 4px 11px 3px #e0e0e078;
            border-radius:20px;
        }


        .product_contect_card {
        ...
        /*DESCRIPCION*/
        /* Diseño para las tarjetas de descripción (Instrucciones, Seguridad, etc.) */
        .description-cards-grid {
            display: grid;
            /* Crea columnas automáticas que no sean menores a 200px. En PC se verán 4, en móviles 1 o 2 */
        ...
        .desc-card-item {
            ...
            transition: all 0.3s ease;
            height: 100%; /* Asegura que todas tengan el mismo alto */
        }

        .desc-card-item:hover {
        ...
        #add_to_cart_btn{border-radius:10px;padding:8px 30px}

        .product_content_details{position:relative}
...
@section('content')
    ...
    <div class="product_details_wrapper vista-interna-producto">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="row">
                                ...
                                <div class="slider-for gallery_large">
                                    ...
                                    @else
                                        <div class="item-slick slick-current slick-active" id="thumb_{{$product->id}}">
                                            <img alt="{{$product->product_name}}" class="varintImg zoom_01" @if ($product->thum_img != null) data-zoom-image="{{showImage($product->thum_img)}}" @else data-zoom-image="{{showImage($product->product->thumbnail_image_source)}}" @endif @if ($product->thum_img != null) src="{{showImage($product->thum_img)}}" @else src="{{showImage($product->product->thumbnail_image_source)}}" @endif title="{{$product->product_name}}">
                                        </div>
                                    @endif
                                </div>
                                <div class="slider-nav">
                                    ...
                                    @else
                                        <div class="item-slick slick-active slick-current">
                                            <img alt="{{$product->product_name}}" @if ($product->thum_img != null) src="{{showImage($product->thum_img)}}" @else src="{{showImage($product->product->thumbnail_image_source)}}" @endif  title="{{$product->product_name}}">
                                        </div>
                                    @endif
                                </div>
                        ...
                        <div class="col-lg-6 col-xl-7">
                            <div class="product_content_details mb_20 product_contect_card">
                                {{-- Product poits --}}
                                @if(isModuleActive('ClubPoint'))
                                <div id="points_badge">
                                    <span class="start"><x-public-point-icon /></span>
                                    <span class="points">{{getNumberTranslate(@$product->product->club_point)}}</span>
                                </div>
                                ...
                                <div class="destils_prise_information_box mb_20">
                                    <div class="d-flex align-items-center flex-wrap gap_15 justify-content-between">
                                        <div>
                                        ...
                                        @if(isGuestAddtoCart() == true)
                                        <div class="platino-premium-block" title="Beneficio exclusivo Plan PLATINO">
                                            <div class="platino-header">
                                                <span><i class="ti-medall"></i> Beneficio Platino</span>
                                                <span class="platino-discount-badge">-20% DTO</span>
                                            </div>
                                            <div class="platino-price-row">
                                                <span class="platino-label">Llevátelo por solo:</span>
                                                <span class="platino-price-big">{{ single_price(@$product->skus->first()->sell_price * 0.8) }}</span>
                                            </div>
                                        </div>
                                        @endif
                                    </div>
                                ...
                                @if(isModuleActive('WholeSale'))
                                    <div class="{{ @$product->skus->where('status',1)->first()->wholeSalePrices->count() ? 'd-flex':'d-none'}}">
                                        <table class="table-sm append_w_s_p_tbl mb-3" style="width:100%">
                                            <thead>
                                ...
                                <div class="product_info row">
                                    <div class="col-12 col-xl-6">
                                        ...
                                        <div class="add_wish_compare flex-column d-flex  flex-wrap mb_20">
                                            <a id="wishlist_btn" data-product_id="{{$product->id}}" data-seller_id="{{$product->user_id}}" class="single_wish_compare text-uppercase text-nowrap cursor_pointer mb-2">
                                                <i class="far fa-heart"></i> {{__('defaultTheme.add_to_wishlist')}}
                                            </a>
                                            <a id="add_to_compare_btn_modify" data-product_sku_id="#product_sku_id" data-product_type="{{$product->product->product_type}}" class="single_wish_compare text-uppercase text-nowrap cursor_pointer mb-2">
                                    ...
                                    <div class="col-12  col-xl-6 mt-xl-0">
                                        @if(isGuestAddtoCart() == true)
                                        <h5 class="mb-0 text-center">{{__('common.total')}}:
                                            <span id="total_price">
                                                @if(@$product->hasDeal)
                                        ...
                                        @if(isGuestAddtoCart() == true)
                                        <div class="row mt-2 " id="add_to_cart_div">
                                                @if ($product->stock_manage == 1 && $product->skus->where('status',1)->first()->product_stock >= $product->product->minimum_order_qty)
                                                    <div class="col-12">
                                                        <button type="button" id="add_to_cart_btn" class="amaz_primary_btn style2 mb_20  add_to_cart add_to_cart_btn text-uppercase flex-fill text-center w-100">{{__('common.add_to_cart')}} <i class="fas fa-shopping-cart"></i></button>
                                                    </div>
                                                    <div class="col-12">
                                                ...
                                                @elseif($product->stock_manage == 0)
                                                    <div class="col-12">
                                                        <button type="button" id="add_to_cart_btn" class="amaz_primary_btn style2 mb_20  add_to_cart text-uppercase add_to_cart_btn flex-fill text-center w-100">{{__('common.add_to_cart')}} <i class="fas fa-shopping-cart"></i></button>
                                                    </div>
                                                    <div class="col-12">
                                                        <button type="button" id="butItNow" class="amaz_primary_btn3 mb_20  w-100 text-center justify-content-center text-uppercase buy_now_btn" data-id="{{$product->id}}" data-type="product">{{__('common.buy_now')}}</button>
                                                    </div>
                         ...
                         <div class="col-12">
                            ...
                            @endif
                            <div class="amazcart_delivery_wiz amazcart_delivery_wiz_1 mb_20">
                                <div class="amazcart_delivery_wiz_head">
                                        ...
                                        <div class="product_details">
                                            ...
                                            @if ($product->product->video_provider == 'daily_motion')
                                                ...
                                                <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;">
                                                    <iframe title="iframe" style="width:100%;height:100%;position:absolute;left:0px;top:0px;overflow:hidden" frameborder="0" type="text/html"
        ...
        $(document).ready(function(){
            ...
            function fetch_data(page){
                ...
                if(page != 'undefined'){
                    ...
                    toastr.warning("{{__('common.undefined_error')}}", "{{__('common.warning')}}");
                }
            }
            ...
            $(document).on('click', '.add_to_cart_btn', function(event){
                ...
                var showData = {
                    ...
                    'points': @json(optional($product->product)->club_point ?? 0),
                };
                ...
                $.post("{{route('frontend.item.get_pickup_info')}}",data,function(response){
                    if (response.shipping != null) {
                        if(response.shipping.cost == 0){
                            $('#door_delivery').text(`
                                ${trans('amazy.free_shipping_note')} ${response.shipping.shipment_time_label ?? response.shipping.shipment_time}.
                            `);
                        }else{
                            $('#door_delivery').text(`
                                ${trans('amazy.shipping_note')} ${currency_format(response.shipping.cost)}. ${trans('amazy.Delivery within')} ${response.shipping.shipment_time_label ?? response.shipping.shipment_time}.
                            `);
                        }
                    }
