                                ...
                                <div class="contact_form_box">
                                    <div class="contact_info">
                                        <div class="contact_title">
                                            <h4 class="">{{ $contactContent->mainTitle }}</h4>
                                        </div>

                                        <div class="contact_description">
                                            {{ !empty($contactContent->subTitle) ? $contactContent->subTitle : $contactContent->description }}
                                        </div>
                                    </div>
                                                ...
                                                @php
                                                    if($row->type != 'header' && $row->type !='paragraph'){
                                                        $default_placeholders = $default_placeholders ?? [
                                                            'name' => __('defaultTheme.enter_name'),
                                                            'email' => __('defaultTheme.enter_email_address'),
                                                            'phone' => __('defaultTheme.phone_number'),
                                                            'phone_number' => __('defaultTheme.phone_number'),
                                                            'message' => __('defaultTheme.write_messages'),
                                                        ];
                                                        ...
                                                        $normalizedFieldName = strtolower(str_replace(['-', ' '], '_', (string) $row->name));
                                                        $normalizedLabel = strtolower(trim((string) ($row->label ?? '')));

                                                        if (array_key_exists($normalizedFieldName, $default_placeholders)) {
                                                            $placeholder = $default_placeholders[$normalizedFieldName];
                                                        } elseif (str_contains($normalizedFieldName, 'phone') || in_array($normalizedLabel, ['phone number', 'numero de telefono', 'número de teléfono', 'telefono', 'teléfono'])) {
                                                            $placeholder = __('defaultTheme.phone_number');
                                                        }
                                                    }
                                                @endphp
                                                    @if($row->type =='header' || $row->type =='paragraph')
                                                        <div class="form-group">
                                                            <{{ $row->subtype }}>{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }} </{{ $row->subtype }}>
                                                        </div>
                                                    @elseif($row->type == 'text' || $row->type == 'number' || $row->type == 'email' || $row->type == 'date')
                                                        <div class="col-xl-12">
                                                            <input {{$required ? 'required' :''}} name="{{$row->name}}" id="{{$row->name}}" placeholder="{{$placeholder}}" onfocus="this.placeholder = ''" onblur="this.placeholder = '{{ $placeholder }}'" class="@error($row->name) is-invalid @enderror primary_line_input style4 mb_20" value="{{ old($row->name) }}" type="{{$type}}">
                                                            @error($row->name)
                                                    ...
                                                    @elseif($row->type=='textarea')
                                                        <div class="col-xl-12">
                                                            <textarea class="form-control primary_line_textarea style4 mb_40" {{$required ? 'required' :''}} name="{{$row->name}}" placeholder="{{$placeholder}}" onfocus="this.placeholder = ''" onblur="this.placeholder = @js($placeholder)" id="{{$row->name}}">{{old($row->name)}}</textarea>
                                                            <span class="text-danger">{{$errors->first($row->name)}}</span>
                                                        </div>
