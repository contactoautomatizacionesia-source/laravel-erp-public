...
@section('profile_content')
    <div class="amazy_dashboard_area dashboard_bg section_spacing6">
        ...
        <div class="cd-recharge-card mb-4">
            {{-- ── Header ── --}}
            <div class="cd-recharge-header">
                <h4>{{ __('customer_panel.my_wallet_recharge') }}</h4>
                <div class="cd-recharge-amount-badge">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                        <line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>
                    </svg>
                    {{ single_price($converted_amount) }}
                </div>
            </div>

            {{-- ── Body: gateway buttons ── --}}
            <div class="cd-recharge-body">
                <p class="cd-recharge-subtitle">{{ __('wallet.select_payment_method') }}</p>

                <div class="cd-gateway-grid">
                    @if (@$payment_gateways->where('method', 'Stripe')->first()->active_status == 1)
                        <form action="{{ route('my-wallet.store') }}" method="post">
                            ...
                            <button class="cd-gateway-btn" type="submit">
                            ...
                            <script src="https://checkout.stripe.com/checkout.js" class="stripe-button"
                                ...
                                data-image="{{ showImage(app('general_setting')->favicon) }}" data-locale="auto"
                        ...
                        <form action="{{ route('my-wallet.store') }}" method="POST">
                            ...
                            <button class="cd-gateway-btn" type="submit">
                                <img src="{{ showImage($gateway_activations->where('method', 'RazorPay')->first()->logo) }}"
                                    alt="RazorPay" title="RazorPay">
                            </button>
                            @csrf
                            @php
                                $razor_credential = getPaymentInfoViaSellerId(1, 'razorpay');
                            @endphp
                            <script src="https://checkout.razorpay.com/v1/checkout.js" data-key="{{ @$razor_credential->perameter_1 }}"
                                data-amount="{{ $recharge_amount * 100 }}"
                                data-name="{{ str_replace('_', ' ', app('general_setting')->company_name) }}" data-description="Wallet Recharge"
                                data-image="{{ showImage(app('general_setting')->favicon) }}" data-prefill.name="{{ auth()->user()->username }}"
                        ...
                        <form action="{{ route('my-wallet.store') }}" method="post">
                            ...
                            <button class="cd-gateway-btn" type="submit">
                                <img src="{{ showImage($gateway_activations->where('method', 'PayStack')->first()->logo) }}"
                                    alt="PayStack" title="PayStack">
                            </button>
                        </form>
                    @endif

                    @if (@$payment_gateways->where('method', 'Bank Payment')->first()->active_status == 1)
                        <button class="cd-gateway-btn" data-bs-toggle="modal" data-bs-target="#exampleModal">
                            <img src="{{ showImage($gateway_activations->where('method', 'Bank Payment')->first()->logo) }}"
                                alt="Bank Payment" title="Bank Payment">
                        </button>
                    @endif

                    @if (@$payment_gateways->where('method', 'PayTM')->first()->active_status == 1)
                        <button class="cd-gateway-btn" data-bs-toggle="modal" data-bs-target="#PayTMModal">
                    ...
                    @if (@$payment_gateways->where('method', 'Instamojo')->first()->active_status == 1)
                        <button class="cd-gateway-btn" data-bs-toggle="modal" data-bs-target="#InstamojoModal">
                            <img src="{{ showImage($gateway_activations->where('method', 'Instamojo')->first()->logo) }}"
                                alt="Instamojo" title="Instamojo">
                        </button>
                    @endif
                    ...
                    @if (@$payment_gateways->where('method', 'FlutterWave')->first()->active_status == 1)
                        <button class="cd-gateway-btn" data-bs-toggle="modal" data-bs-target="#FlutterWaveModal">
                            <img src="{{ showImage($gateway_activations->where('method', 'FlutterWave')->first()->logo) }}"
                                alt="FlutterWave" title="FlutterWave">
                        </button>
                    @endif

                    @if (isModuleActive('MercadoPago') && @$payment_gateways->where('method', 'Mercado Pago')->first()->active_status == 1)
                        <button class="cd-gateway-btn" data-bs-toggle="modal" data-bs-target="#MercadoPagoModal">
                            <img src="{{ showImage($gateway_activations->where('method', 'Mercado Pago')->first()->logo) }}"
                    ...
                    @if (isModuleActive('Tabby') && @$payment_gateways->where('method', 'Tabby')->first()->active_status == 1)
                        <form action="{{ route('my-wallet.store') }}" method="post">
                            ...
                            <button class="cd-gateway-btn" type="submit">
                                <img src="{{ showImage($gateway_activations->where('method', 'Tabby')->first()->logo) }}"
                                    alt="Tabby" title="Tabby">
                            </button>
                        </form>
                    @endif

                    @if (isModuleActive('CCAvenue') && @$payment_gateways->where('method', 'CCAvenue')->first()->active_status == 1)
                        <button class="cd-gateway-btn" data-bs-toggle="modal" data-bs-target="#CCAvenueModal">
                            <img src="{{ showImage($gateway_activations->where('method', 'CCAvenue')->first()->logo) }}"
                                alt="CCAvenue" title="CCAvenue">
                        </button>
                    @endif

                    @if (isModuleActive('Bkash') && @$payment_gateways->where('method', 'Bkash')->first()->active_status == 1)
                        @php
                            $credential = getPaymentInfoViaSellerId(1, 'bkash');
                        @endphp
                        @include(theme('pages.profile.wallets.components._bkash'), [
                    ...
                    @if (isModuleActive('Clickpay') && @$payment_gateways->where('method', 'Clickpay')->first()->active_status == 1)
                        @include('clickpay::wallet_recharge', compact('converted_amount'))
                    @endif
                </div>

                {{-- ── Back to wallet ── --}}
                <a href="{{ route('my-wallet.index', 'customer') }}" class="cd-back-link">
        ...
        (function($) {
            ...
            $(document).ready(function() {
                ...
                $(document).on('submit', '#bank_payment_form', function(event) {
                    ...
                    let name = $(
                            '#bank_payment_form > div > div:nth-child(3) > div:nth-child(1) > input')
                        .val();
                    let branch_name = $(
                            '#bank_payment_form > div > div:nth-child(3) > div:nth-child(2) > input')
                        .val();
                    let account_number = $(
                            '#bank_payment_form > div > div:nth-child(4) > div:nth-child(1) > input')
                        .val();
                    let account_holder = $(
                            '#bank_payment_form > div > div:nth-child(4) > div:nth-child(2) > input')
                        .val();
                ...
                $(document).on('submit', '#paytm_form', function(event) {
                    ...
                    if (name == '') {
                        $('#paytm_form > div:nth-child(3) > div:nth-child(1) > span').text(
                            "{{ __('validation.this_field_is_required') }}");
                        val_check = 1;
                    }
                    if (email == '') {
                        $('#paytm_form > div:nth-child(3) > div:nth-child(2) > span').text(
                ...
                $(document).on('submit', '#instamojo_form', function(event) {
                    ...
                    let email = $('#instamojo_form > div:nth-child(3) > div:nth-child(2) > input')
                .val();
                ...
                $(document).on('submit', '#flatter_wave_form', function(event) {
                    ...
                    let name = $('#flatter_wave_form > div:nth-child(3) > div:nth-child(1) > input')
                        .val();
                    let email = $('#flatter_wave_form > div:nth-child(3) > div:nth-child(2) > input')
                        .val();
                    let mobile = $('#flatter_wave_form > div.row.mb-20 > div:nth-child(1) > input')
                    .val();
                    let amount = $('#flatter_wave_form > div.row.mb-20 > div:nth-child(2) > input')
                    .val();
