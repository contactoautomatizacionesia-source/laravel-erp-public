...
@push('styles')
<style>
    .lif-diff-card{background:#fff;border:1px solid #eef0f4;border-radius:14px;padding:16px;margin-bottom:18px}
    .lif-diff-header{display:flex;justify-content:space-between;align-items:center;gap:10px}
    .lif-diff-title{font-size:15px;font-weight:700;margin:0}
    .lif-diff-stepper{display:flex;flex-direction:column;gap:14px;margin-top:14px}
    .lif-diff-step{display:flex;gap:12px;align-items:flex-start;position:relative}
    .lif-diff-step::before{content:'';position:absolute;left:14px;top:34px;width:2px;height:calc(100% - 22px);background:#e5e7eb}
    .lif-diff-step:last-child::before{display:none}
    .lif-diff-avatar{width:30px;height:30px;border-radius:10px;background:#1f2a44;color:#fff;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700}
    .lif-diff-box{flex:1;background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:10px 12px}
    .lif-diff-box.dark{background:#0f172a;color:#fff;border-color:#0f172a}
    .lif-diff-box.gray{background:#f3f4f6;color:#6b7280}
    .lif-diff-row{display:flex;justify-content:space-between;gap:10px;font-size:12px}
    .lif-diff-row strong{font-size:13px}
    .lif-diff-tag{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:2px 8px;font-size:10px;background:#e2e8f0;color:#0f172a}
</style>
@endpush
@section('content')
    <div class="amazy_dashboard_area dashboard_bg section_spacing6 ign-my-purchase-order-details-profile">
        <div class="container">
            <div class="row">
                <div class="col-xl-8 col-lg-8">
                    ...
                    <div class="white_box style2 bg-white mb_30">
                        <div class="white_box_header gray_color_1 d-flex align-items-center gap_20 flex-wrap  theme_border justify-content-between ">
                                ...
                                <div class="d-flex align-items-center flex-wrap gap_5">
                                    ...
                                    @php
                                        if ($order->is_cancelled == 1) {
                                            $orderStatusText = __('common.cancelled');
                                            $orderStatusClass = 'badge_2';
                                        } elseif ($order->is_completed == 1) {
                                            $orderStatusText = __('common.completed');
                                            $orderStatusClass = 'badge_1';
                                        } elseif ($order->is_confirmed == 1) {
                                            $orderStatusText = __('common.confirmed');
                                            $orderStatusClass = 'badge_1';
                                        } elseif ($order->is_confirmed == 2) {
                                            $orderStatusText = __('common.declined');
                                            $orderStatusClass = 'badge_2';
                                        } else {
                                            $orderStatusText = __('common.pending');
                                            $orderStatusClass = 'badge_3';
                                        }
                                    @endphp
                                    <p class="{{ $orderStatusClass }}">
                                        {{ $orderStatusText }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="d-flex flex-column  ">
                                <a  href="{{ route('frontend.my_purchase_order_pdf', encrypt($order->id)) }}" target="_blank" class="btn-toolkit btn-primary"> {{__('defaultTheme.download_invoice')}}</a>
                            </div>
                        </div>
                        <div class="dashboard_white_box_body dashboard_orderDetails_body">
                            ...
                            @endphp
                            <div class="lif-diff-card" id="differentials">
                                <div class="lif-diff-header">
                                    <h4 class="lif-diff-title">{{ __('tree.differentials_distribution') }}</h4>
                                    <span class="lif-diff-tag">{{ __('tree.open') }}</span>
                                </div>
                            ...
                            @foreach ($order->packages as $key => $package)
                                <div class="order_prise d-flex justify-content-between gap-2 flex-wrap amazy_bb2 pb_11 mb_10">
                                    <h4 class="font_16 f_w_700 m-0">{{__('common.package')}} : {{ getNumberTranslate($package->package_code) }}</h4>
                                    ...
                                    <h4 class="font_16 f_w_700 m-0">{{__('common.price')}}: {{ single_price($package->products->sum('total_price') + $package->gst_taxes->sum('amount')) }}</h4>
                                    @else
                                    ...
                                    @endif
                                    @if($package->delivery_status==4 && $oneClickOrderComplete->status==1)
                                        <div class="form-check">
                                            <label class="primary_checkbox d-flex">
                                                ...
                                                <span class="label_name f_w_700">{{__('setup.Is Received?')}}</span>
                                            </label>
                                        </div>
                                    @endif
                                </div>
                                @if($package->is_cancelled == 0)
                                    <p class="font_14 f_w_400">{{ $package->formattedShippingDate() }}</p>
                                                ...
                                                @if ($package->delivery_status >= 1)
                                                    <svg  width="30" height="30" viewBox="0 0 30 30">
                                                        <g  data-name="1" transform="translate(-613 -335)">
                                                            <circle  data-name="Ellipse 239" cx="15" cy="15" r="15" transform="translate(613 335)" fill="#89a021"></circle>
                                        ...
                                        <div class="single_order_progress position-relative d-flex align-items-center flex-column">
                                            <div class="icon position-relative order_process_icon">
                                                ...
                                                @elseif($package->delivery_status >= 1 && !$ready_to_ship)
                                                    <svg  width="30" height="30" viewBox="0 0 30 30">
                                                        <g  data-name="1" transform="translate(-613 -335)">
                                                            <g  data-name="Ellipse 239" transform="translate(613 335)" fill="none" stroke="#89a021" stroke-width="2">
                                                            <circle cx="15" cy="15" r="15" stroke="none"></circle>
                                                            <circle cx="15" cy="15" r="14" fill="none"></circle>
                                                            </g>
                                                            <circle  data-name="Ellipse 240" cx="5" cy="5" r="5" transform="translate(623 345)" fill="#89a021"></circle>
                                                        </g>
                                                    </svg>
                                                @else
                                                    <svg  width="30" height="30" viewBox="0 0 30 30">
                                    ...
                                    <div class="d-flex align-items-center gap_20 flex-wrap gray_color_1 dashboard_orderDetails_head  justify-content-between theme_border">
                                        <div class="d-flex flex-column ">
                                            <div class="d-flex align-items-center flex-wrap gap_5">
                                                <h4 class="font_14 f_w_500 m-0 lh-base">{{__('defaultTheme.package_code')}}:  </h4> <p class="font_14 f_w_400 m-0 lh-base"> {{ getNumberTranslate($package->package_code) }}</p>
                                            </div>
                                        </div>
                                        <div class="d-flex flex-column  ">
                                            <div class="d-flex align-items-center flex-wrap gap_5">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <a href="{{route('frontend.gift-card.show',@$package_product->giftCard->sku)}}" class="d-flex align-items-center gap_20 cart_thumb_div">
                                                                <div class="thumb">
                                                                    <img class="img-res" src="{{showImage(@$package_product->giftCard->thumbnail_image)}}" alt="{{textLimit(@$package_product->giftCard->name,28)}}" title="{{textLimit(@$package_product->giftCard->name,28)}}">
                                                                </div>
                                                                <div class="summery_pro_content">
                                                                    <h4 class="font_16 f_w_700 text-nowrap m-0 secondary_text">{{textLimit(@$package_product->giftCard->name,28)}}</h4>
                                                                    ...
                                                                    <p>{{__('common.qty')}}: {{ $package_product->qty }}</p>
                                                                    <p>{{__('common.price')}}: {{ single_price($package_product->price) }}</p>
                                                                    <p>{{__('common.points')}}: -</p>
                                                                </div>
                                                            </a>
                                                        </td>
                                                        <td></td>
                                                            ...
                                                            <a href="{{singleProductURL(@$package_product->seller_product_sku->product->seller->slug, @$package_product->seller_product_sku->product->slug)}}" class="d-flex align-items-center gap_20 cart_thumb_div">
                                                                ...
                                                                <div class="summery_pro_content">
                                                                    ...
                                                                    @if (@$package_product->seller_product_sku->product->is_digital)
                                                                        <a download="" class="font_14 f_w_400 m-0" target="_blank" href="{{ route('digital_file_download', encrypt($package->files->where('product_sku_id', $package_product->seller_product_sku->product_sku_id)->where('customer_id', auth()->user()->id)->first()->id)) }}"><i class="ti-download mr-1 green"></i>
                                                                            {{__('common.download')}}
                                                                        </a>
                                                                    @endif
                                                                    ...
                                                                    <p>
                                                                        {{__('common.points')}}:
                                                                        {{ ($package_product->total_club_point ?? 0) > 0 ? getNumberTranslate($package_product->total_club_point) : '' }}
                                                                    </p>
                                                                </div>
                                                            </a>
                                                        </td>
                                    ...
                                    <div class="d-flex flex-column mt_10">
                                        <div class="d-flex align-items-center flex-wrap gap_5 justify-content-end">
                                            @if ($order->is_confirmed == 0 && $package->is_cancelled == 0 || $order->is_confirmed == 1 && $package->is_cancelled == 0 && $package->delivery_status <= 2)
                                                <a href="" data-id={{ $package->id }} class=" order_cancel_by_id btn-toolkit btn-danger">{{__('defaultTheme.cancel_order')}}</a>
                                            @elseif ($order->is_completed == 1 || $package->delivery_status >= 5)
                                                @php
                                                    $refund_status=app('business_settings')->where('type', 'refund_status')->first()->status;
                                                @endphp
                                                @if (\Carbon\Carbon::now() <= $order->created_at->addDays(app('business_settings')->where('type', 'refund_times')->first()->status)  && $package->is_cancelled == 0 && $physical_product == 1 && !$package->refundPackage && $refund_status == 1)
                                                    <a href="{{ route('refund.make_request', encrypt($package->id)) }}" class="btn-toolkit btn-secondary">{{__('defaultTheme.open_dispute')}}</a>
                                                @endif
                                            @endif
                                            @if($order->is_completed == 1)
                                                <a href="{{url('/')}}/profile/product-review?order_id={{base64_encode($order->id)}}&&package_id={{base64_encode($package->id)}}&&seller_id={{base64_encode($package->seller_id)}}" class="btn-toolkit btn-primary">{{__('defaultTheme.write_a_review')}}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            @endif
                            <div class="order_details_list_box mt-3">
                                <div class="summery_order_body d-flex flex-wrap">
                                    <div class="summery_lists flex-fill p-2">
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 ">
                                                <h4 class="font_16 f_w_700 text-nowrap m-0 secondary_text">{{__('shipping.shipping_info')}} @if($order->delivery_type == 'pickup_location')({{__('shipping.collect_from_pickup_location')}}) @endif</h4>
                                            </div>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.name')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->shipping_name}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.email')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->shipping_email}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.phone_number')}} </h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{getNumberTranslate(@$order->address->shipping_phone)}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.address')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->shipping_address}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.state')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->getShippingCity->name}}, {{@$order->address->getShippingState->name}}, {{@$order->address->getShippingCountry->name}}</p>
                                        </div>
                                    ...
                                    <div class="summery_lists flex-fill p-2">
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 ">
                                                <h4 class="font_16 f_w_700 text-nowrap m-0 secondary_text">{{__('shipping.billing_info')}} </h4>
                                            </div>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.name')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->billing_name}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.email')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->billing_email}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.phone_number')}} </h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{getNumberTranslate(@$order->address->billing_phone)}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.address')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->billing_address}}</p>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.state')}}</h5>
                                            </div>
                                            <p class="font_14 f_w_400 m-0">{{@$order->address->getBillingCity->name}}, {{@$order->address->getBillingState->name}}, {{@$order->address->getBillingCountry->name}}</p>
                                        </div>



                                        <div class="border rounded shadow-sm p-3 mt-5">

                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 ">
                                                    <h4 class="font_16 f_w_700 text-nowrap m-0 secondary_text">{{__('defaultTheme.payment_info')}} </h4>
                                                </div>
                                            </div>
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.subtotal')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0">{{single_price($order->sub_total)}}</p>
                                            </div>
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.discount')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0">- {{single_price($order->discount_total)}}</p>
                                            </div>
                                            @if($order->coupon)
                                                <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                    <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.coupon')}}</h5>
                                                    </div>
                                                    <p class="font_14 f_w_400 m-0">- {{single_price($order->coupon->discount_amount)}}</p>
                                                </div>
                                            @endif
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('defaultTheme.tax_amount')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0">+ {{single_price($order->tax_amount)}}</p>
                                            </div>
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.shipping_charge')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0"> + {{single_price($order->shipping_total)}}</p>
                                            </div>
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.grand_total')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0">{{single_price($order->grand_total)}}</p>
                                            </div>
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.totals_points')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0">{{ $order->total_points }}</p>
                                            </div>
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('defaultTheme.paid_by')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0">{{$order->GatewayName}}</p>
                                            </div>
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.txn_id')}}</h5>
                                                </div>
                                            ...
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 text-nowrap"><h5 class="font_14 f_w_500 m-0">{{__('defaultTheme.payment_status')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0">
                                                    @if ($order->is_paid == 1)
                                                        {{__('common.paid')}}
                                            ...
                                            <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                                <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.payment_reference')}}</h5>
                                                </div>
                                                <p class="font_14 f_w_400 m-0"> - </p>
                                            </div>
                                        </div>
                ...
                <div class="col-xl-4 col-lg-4">
                    <div class="dashboard_white_box style3 rounded-0 bg-white mb_20">
                        <div class="description_box">
                            <h4 class="font_20 f_w_600 mb-2 secondary_text">{{__('common.advised_by')}}</h4>
                            <ul class="text-secondary">
                                <li class="">{{__('common.name')}}: <span class="font-weight-bold">-</span></li>
                                <li class="">{{__('common.date')}}: <span class="font-weight-bold">-</span></li>
                                <li class="">{{__('common.channel ')}}: <span class="font-weight-bold">-</span></li>
                            </ul>
                        </div>
                    </div>
                    <div class="dashboard_white_box style3 rounded-0 bg-white mb_20">
                        <div class="description_box">
                            <h4 class="font_20 f_w_600 mb-2 secondary_text">{{__('common.cost_center')}}</h4>
                            <h4>Armenia</h4>
                            <p class="text-black">Centro comercial premium planza — Local 1119 Cr 43A # 30-25</p>
                        </div>
                    </div>
                    <hr>
                    ...
                    @foreach ($processes as $key => $process)
                        <div class="dashboard_white_box style3 rounded-0 bg-white mb_20">
                            ...
                            <h4 class="font_20 f_w_600 mb-2 secondary_text">{{ $process->name }}</h4>
        ...
        <!-- cancel order modal -->
        <div class="modal fade login_modal about_modal" id="orderCancelReasonModal" tabindex="-1"  aria-labelledby="asq_about_form" aria-hidden="true">
                    ...
                    <!-- infix_login_area::start  -->
                        <div class="infix_login_area p-0">
                            <div class="login_area_inner">
                                ...
                                <form action="{{route('frontend.my_purchase_order_package_cancel')}}" method="post" id="order_cancel_form">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-12 mb_30">
                                            <div class="form-group input_div_mb">
                                                <label class="mb-2" for="reason">{{ __('refund.reason') }} <span>*</span></label>
                                                <select class="style2 wide" name="reason" id="reason" autocomplete="off">
                                        ...
                                        <div class="col-12 text-center">
                                            <button type="submit" class="btn-toolkit btn-primary">{{ __('common.send') }}</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
           ...
           "use strict";
            ...
            $(document).ready(function() {
                ...
                $(document).on('click','#is_received', function(){
                    ...
                    $("#pre-loader").show();
                    $.post('{{ route('change_receive_status_by_customer') }}', {_token:'{{ csrf_token() }}', order_id:order_id, package_id:package_id}, function(data){
                        if (data == 1) {
