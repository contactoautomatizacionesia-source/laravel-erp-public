                    ...
                    <table class="table amazy_table style5 mb-0">
                        ...
                        <tbody>
                            @foreach ($notifications as $notification)
                                <tr>
                                    <td>
                                        <span class="font_14 f_w_500 mute_text">{{ getNumberTranslate($loop->index+1) }}</span>
                                    </td>
                                    <td>
                                        <span class="font_14 f_w_500 mute_text">{{ ucfirst($notification->title) }}</span>
                                    </td>
                                    <td>
                                        <span class="font_14 f_w_500 mute_text">{{ dateConvert($notification->created_at) }}</span>
                                    </td>
                                    <td>
                                        @if ( str_contains("#",$notification->url) == false  &&   $notification->url != null)
                                            <a href="{{url('/').$notification->url}}" class="amaz_badge_btn4 text-nowrap text-capitalize text-center">{{__('common.view')}}</a>
                                        @else
                                            <a href="javascript:void(0)" disabled class="amaz_badge_btn4 text-nowrap text-capitalize text-center disabled" >{{__('common.view')}}</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
