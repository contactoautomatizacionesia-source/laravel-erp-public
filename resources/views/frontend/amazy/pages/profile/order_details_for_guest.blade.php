...
@endpush
@section('content')
    <div class="amazy_dashboard_area dashboard_bg section_spacing6 ign_my_purchase_order_detail_guest">
        <div class="container">
            <div class="row">
                <div class="col-xl-8 col-lg-8">
                    ...
                    <div class="white_box style2 bg-white mb_30">
                        <div class="white_box_header gray_color_1 d-flex align-items-center gap_20 flex-wrap  theme_border justify-content-between ">
                            ...
                            <div class="d-flex flex-column  ">
                                <a href="{{ route('frontend.my_purchase_order_pdf', encrypt($order->id)) }}" target="_blank" class="amaz_primary_btn py-2 radius_3px mb_5">{{__('defaultTheme.download_invoice')}}</a>
                            </div>
                        </div>
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <a href="{{route('frontend.gift-card.show',@$package_product->giftCard->sku)}}" class="d-flex align-items-center gap_20">
                                                                <div class="thumb img-res-div">
                                                                    <img class="img-res img-size" src="{{showImage(@$package_product->giftCard->thumbnail_image)}}" alt="{{textLimit(@$package_product->giftCard->name,22)}}" title="{{textLimit(@$package_product->giftCard->name,22)}}">
                                                                </div>
                                                                <div class="summery_pro_content img-res-div">
                                                                    ...
                                                                    <p>{{__('common.qty')}}: {{ $package_product->qty }}<br>  {{__('common.price')}}: {{ single_price($package_product->price) }}</p>
                                                                </div>
                                                            </a>
                                                        </td>
                                                        <td></td>
                                                            ...
                                                            <a href="{{singleProductURL(@$package_product->seller_product_sku->product->seller->slug, @$package_product->seller_product_sku->product->slug)}}" class="d-flex align-items-center gap_20">
                                                                ...
                                                                <div class="summery_pro_content img-res-div">
                                                                    ...
                                                                    @if (@$package_product->seller_product_sku->product->is_digital)
                                                                        <a  class="font_14 f_w_400 m-0" download="" target="_blank" href="{{ route('digital_file_download', encrypt($package->files->where('product_sku_id', $package_product->seller_product_sku->product_sku_id)->where('customer_id', auth()->user()->id)->first()->id)) }}"><i class="ti-download mr-1 green"></i>
                                                                            {{__('common.download')}}
                                                                        </a>
                                                                    @endif
                                                                    ...
                                                                    @endif
                                                                    <p>
                                                                        {{__('common.qty')}}: {{ $package_product->qty }} <br>
                                                                        {{__('common.price')}}: {{ single_price($package_product->price) }} <br>
                                                                        {{__('common.totals_points')}}: {{$package_product->qty * $package_product->unit_club_point}}
                                                                    </p>
                            ...
                            <div class="d-flex flex-column mt_10">
                                <div class="d-flex align-items-center flex-wrap gap_5 justify-content-end">
                                    @if ($order->is_confirmed == 0 && $package->is_cancelled == 0 || $order->is_confirmed == 1 && $package->is_cancelled == 0 && $package->delivery_status <= 2)
                                        <a href="" data-id={{ $package->id }} class="amaz_primary_btn2 radius_3px order_cancel_by_id py-2">{{__('defaultTheme.cancel_order')}}</a>
                                    @elseif ($order->is_completed == 1 || $package->delivery_status >= 5)
                                        @if (\Carbon\Carbon::now() <= $order->created_at->addDays(app('business_settings')->where('type', 'refund_times')->first()->status) && $package->is_cancelled == 0 && $physical_product == 1 && !$package->refundPackage)
                                            <a href="{{ route('refund.make_request', encrypt($package->id)) }}" class="amaz_primary_btn2 py-2 radius_3px">{{__('defaultTheme.open_dispute')}}</a>
                                        @endif
                                    @endif
                                    @if($package->delivery_status > 4 && count(@$package->reviews) < 1 && $order->is_completed == 1)
                                        <a href="{{url('/')}}/profile/product-review?order_id={{base64_encode($order->id)}}&&package_id={{base64_encode($package->id)}}&&seller_id={{base64_encode($package->seller_id)}}" class="amaz_primary_btn py-2 radius_3px">{{__('defaultTheme.write_a_review')}}</a>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="summery_order_body d-flex flex-wrap">
                                    <div class="summery_lists flex-fill p-2">
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 ">
                                                <h4 class="font_16 f_w_700 text-nowrap m-0 secondary_text">{{__('defaultTheme.shipping_info')}} @if($order->delivery_type == 'pickup_location'){{__('shipping.collect_from_pickup_location')}} @endif</h4>
                                            </div>
                                        </div>
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            <div class="order_text_head d-flex align-items-center  font_14 f_w_500 "><h5 class="font_14 f_w_500 m-0">{{__('common.secret_id')}}</h5>
                                            </div>
                                        ...
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            ...
                                            <p class="font_14 f_w_400 m-0">{{@$order->guest_info->getShippingCity->name}}, {{@$order->guest_info->getShippingState->name}}, {{@$order->guest_info->getShippingCountry->name}}</p>
                                        </div>
                                    ...
                                    <div class="summery_lists flex-fill p-2">
                                        ...
                                        <div class="single_summery_list d-flex align-items-start gap_20 justify-content-between">
                                            ...
                                            <p class="font_14 f_w_400 m-0">{{@$order->guest_info->getBillingCity->name}}, {{@$order->guest_info->getBillingState->name}}, {{@$order->guest_info->getBillingCountry->name}}</p>
                                        </div>
                ...
                <div class="col-xl-4 col-lg-4">
                    ...
                    @foreach ($processes as $key => $process)
                        <div class="dashboard_white_box style3 rounded-0 bg-white mb_20">
                            ...
                            <h4 class="font_20 f_w_700 mb-2 secondary_text">{{ $process->name }}</h4>
