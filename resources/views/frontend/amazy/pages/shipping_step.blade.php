...
@section('content')
    ...
    <form class="shiping-info" action="{{route('frontend.checkout')}}" method="GET" enctype="multipart/form-data" id="mainOrderForm">
                                                ...
                                                <div class="ck-cart-box">
                                                        ...
                                                        <select name="intshipping_cartItem[]" id="uniqueCartId{{$shipcart->id}}" class="theme_select w-100 ck-select intshiping" data-id="{{$shipcart->id}}">
                                                            ...
                                                            @foreach ($rates as $rate)
                                                                ...
                                                                @if ($rate->base_on_item == 1)
                                                                        ...
                                                                        @php
                                                                             $product_shipping_cost = ($shipcart->product->sku->weight * $shipcart->qty / 100) * $rate->rate_cost + ($shipcart->product->sku->additional_shipping * $shipcart->qty);
                                                                        @endphp
                                                                ...
                                                                @elseif ($rate->base_on_item == 2)
                                                                        ...
                                                                        @php
                                                                        $product_shipping_cost = ($shipcart->total_price / 100) * $rate->rate_cost + ($shipcart->product->sku->additional_shipping * $shipcart->qty);
                                                                        @endphp
                                                                        <option value="{{($product_shipping_cost)}} {{$rate->id}}">{{$rate->rate_name}} - {{single_price($product_shipping_cost)}} - {{$rate->shipment_time_label}}</option>
                                                    ...
                                                    <div class="w-100 mt-3">
                                                        <div class="alert alert-warning">
                                                            <p>This Item can not be shipped in your country.</p>
                                                        </div>
                                                    </div>
                                                    @endif
                                                </div>
                                                            ...
                                                            <label class="primary_checkbox d-inline-flex style4 flex-wrap">
                                                                ...
                                                                <span class="label_name f_w_500 mute_text ml_15 text-nowrap">
                                                                    [
                                                                        ...
                                                                        {{$shipping->shipment_time_label}} -

                                                                        [<span class="required_mark_theme">
                                                                        @if($shipping->cost_based_on == 'Price')
                                                                            {{ __('common.per_hundred') }}
                                                                        @elseif($shipping->cost_based_on == 'Weight')
                                                                            {{ __('common.per_100_gm') }}
                                                                        @else
                                                                            {{ __('common.flat_rate') }}
                                                                        @endif
                                                                        </span>]
                                                        ...
                                                        <label class="primary_checkbox d-inline-flex style4">
                                                            ...
                                                            <span class="label_name f_w_500 mute_text ml_15">
                                                                [
                                                                    ...
                                                                    {{$email_shipping->shipment_time_label}}
                                                                ]
                                                            </span>
                                                        </label>
                                ...
                                @endif
                                    ...
                                    <div class="col-12">
                                        <div class="check_v3_btns flex-wrap d-flex align-items-center justify-content-center justify-content-md-start">
                                            <a href="{{url('/checkout')}}" class="amaz_outline_btn">{{__('defaultTheme.return_to_information')}}</a>
                                            @if(isModuleActive('INTShipping'))
                                              <button {{ in_array(0,$can_process) ? 'disabled':'' }} type="submit" class="amaz_primary_btn style3 px-4 {{ in_array(0,$can_process) ? 'disabled':'' }} min_200 text-center text-uppercase" id="IntshippingBtn">{{__('defaultTheme.continue_to_payment')}}</button>
                                            @else
