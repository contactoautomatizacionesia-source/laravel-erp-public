...
@endif
    <div class="amazy_login_area bg-custom-login">
        @php
                            ...
                            @foreach($form_data as $row)
                                ...
                                @elseif($row->type == 'text' || $row->type == 'number' || $row->type == 'email' || $row->type == 'date')
                                    <div class="col-xl-12 mb_20">
                                        <label class="primary_label2" for="{{$row->name}}">{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }} @if($required) <span>*</span> @endif </label>
                                        <input {{$required ? 'required' :''}} type="{{$type}}" id="{{$row->name}}" name="{{$row->name}}" value="{{ old($row->name) }}" placeholder="{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }}" class="@error($row->name) is-invalid @enderror primary_input3 radius_5px">
                                        @error($row->name)
                                            <span class="text-danger" >{{ $message }}</span>
                                        @enderror
                                    </div>
                                        ...
                                        @if (session()->has('pricing_id'))
                                            <div class="col-xl-12 mb_20">
                                                <div class="form-group input_div_mb">
                                                    <label class="primary_label2 style4" for={{$row->name}}> {{ translateWithFallback('formBuilder.'.$row->label, $row->label) }} @if($required) <span>*</span> @endif</label>
                                ...
                                @elseif($row->type == 'date')
                                    <div class="col-12 mb_30">
                                        <label class="primary_label2 style2 " for="start_datepicker">{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }} @if($required) <span>*</span> @endif</label>
                                        <input id="start_datepicker" {{$required ? 'required' :''}} type="{{$type}}" name="{{$row->name}}" value="{{ old($row->name) }}" placeholder="{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }}" class="primary_input3 style4 mb-0 @error($row->name) is-invalid @enderror">
                                        @error($row->name)
                                        <span class="text-danger" >{{ $message }}</span>
                                        @enderror
                                    </div>
                                @elseif($row->type=='textarea')
                                    <div class="col-12 mb_20">
                                        <label class="primary_label2 style2 " for={{$row->name}}>{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }} @if($required) <span>*</span> @endif</label>
                                        <textarea {{$required ? 'required' :''}} name="{{$row->name}}" id="{{$row->name}}"  placeholder="{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }}" class="primary_textarea4 radius_5px primary_input3 radius_5px">{{old($row->name)}}</textarea>
                                    </div>
                                    @elseif($row->type=="radio-group")
                                    <div class="col-lg-12 form-group  mt-10 mb-10">
                                        <label for="">{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }}</label>
                                ...
                                @elseif($row->type=="checkbox-group")
                                    <div class="col-lg-12 form-group mt-10 mb-10">
                                        <label>{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }}</label>
                                        <div class="checkbox">
                                            @foreach($row->values as $value)
                                ...
                                @elseif($row->type =='file')
                                    ...
                                    <div class="col-md-12 mb_20 d-flex ">
                                        <div class="customer_img">
                                            <label for={{$row->name}}>{{ translateWithFallback('formBuilder.'.$row->label, $row->label) }} @if($required) <span class="text-danger">*</span> @endif</label>
