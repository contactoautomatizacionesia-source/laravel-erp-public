...
@section('profile_content')
    <div class="cd-recharge-card mb-4">
        ...
        <div class="cd-recharge-header">
            <h4>{{ __('clubpoint.my_points') }}</h4>
        </div>

        {{-- ── Body ── --}}
        <div class="cd-recharge-body text-center">
            {{-- Exchange rate chip --}}
            <div class="cd-points-row justify-content-center mb-3">
                <div class="cd-points-chip" style="max-width: 360px;">
                    <p class="label">{{ __('clubpoint.exchange_rate') }}</p>
                    <p class="value" style="font-size: 20px;">
                        1 {{ __('clubpoint.point') }}
                        <span style="color:#bbb; font-weight:400; font-size:16px;"> = </span>
                        {{ single_price($wallet_point->wallet_point) }} {{ __('clubpoint.wallet_money') }}
                    </p>
                </div>
            </div>

            <p style="font-size: 12px; color: #aaa; margin: 0;">
                {{ __('clubpoint.piint_convert_refund_date_over') }}
            </p>
        </div>
    </div>
                        ...
                        @foreach(@$orders as $key => $order)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                    <a href="{{route('clubpoint.point',$order->order_id)}}" class="amaz_primary_btn style3 text-uppercase Converted_btn" onclick="$('#pre-loader').show()">{{__('clubpoint.converted_now')}}</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
