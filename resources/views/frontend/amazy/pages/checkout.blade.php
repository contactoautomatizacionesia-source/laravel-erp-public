...
<link href="https://fonts.googleapis.com/css?family=Roboto:400,500" rel="stylesheet"/>
    <style>
          @media (max-width: 400px){
            .shipping_address_div table tbody tr {
                display: flex;
                flex-direction: column;
            }
            .shipping_address_div table tbody tr td:first-child {
                color: #a3a3a3;
            }
          }
...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.saveAddress', function(e){
            ...
            let phone_max = parseInt($("#phone").attr('data-max'));
            let phone_min = parseInt($("#phone").attr('data-min'));
            if(phone_digit.length < phone_min || phone_digit.length > phone_max){
              is_submit = 1;
              $("#error_phone").text("{{ __('common.invalid_phone_number') }} " + phone_min + " {{ __('common.and_maximum') }} " + phone_max + " {{ __('common.digits') }}");
            }
        ...
        $(document).on('change', 'input[name=delivery_type]', function(){
            ...
            if(delivery_type == 'pickup_location'){
                $('#next_step_btn_div').html(
                    ...
                    <button type="submit" class="amaz_primary_btn style3  text-center text-uppercase ">{{__('defaultTheme.continue_to_payment')}}</button>
                    `
                );
                ...
                $('#next_step_btn_div').html(
                    ...
                    <button type="submit" class="amaz_primary_btn style3 text-center text-uppercase ">{{__('defaultTheme.continue_to_shipping')}}</button>
                    `
                );
        ...
        $(document).on('submit', '#mainOrderForm', function(event){
            ...
            if(is_submit === 1){
                ...
                // Mostrar el formulario de edición para que el usuario vea los errores
                $('.shipping_address_div').addClass('d-none');
                $('.shipping_address_edit_div').removeClass('d-none');
                // Hacer scroll al primer error visible
                let firstError = $('.text-danger:not(:empty)').first();
        ...
        // Al cargar la página, si State ya tiene valor pre-seleccionado, cargar las ciudades y pre-seleccionar la guardada
        @if($shipping_address && $shipping_address->state && $shipping_address->city)
        (function(){
            let state = "{{ $shipping_address->state }}";
            let savedCity = "{{ $shipping_address->city }}";
            if(state){
                let base_url = $('#url').val();
                let url = base_url + '/get-city?state_id=' + state;
                $('#city').empty();
                ...
                $.get(url, function(data){
                    $.each(data, function(index, cityObj) {
                        let selected = (cityObj.id == savedCity) ? ' selected' : '';
