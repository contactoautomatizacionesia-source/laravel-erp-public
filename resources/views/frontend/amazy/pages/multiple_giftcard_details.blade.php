                        ...
                        <div class="product_content_details mb_20">
                            ...
                            <div class="destils_prise_information_box mb_20">
                                <h2 class="pro_details_prise d-flex align-items-center  m-0">
                                    <span class="showAmountOfGiftCard">
                                        @foreach ($card->addGiftCard as $m => $value_name)
                                            @if($m === 0)
                                                {{getGiftcardMultiwithDiscountPrice($value_name)}}
                                            @endif
                                        @endforeach
                                    </span>
                                </h2>
                                @if(isGuestAddtoCart())
                                    @foreach ($card->addGiftCard as $m => $value_data)
                                        @if(@$value_data->hasDiscount())
                                            @if($m === 0)
                                                <div class="showDiscountGiftCard">
                                                    <div class="pro_details_disPrise d-flex align-items-center gap_15">
                                                        <h4 class="discount_prise  m-0  "> <span class="text-decoration-line-through"> {{single_price($value_data->gift_selling_price)}}</span> </h4>
                                                        <span class="diccount_percents">
                                                            @if($value_data->gift_discount_type === 0)
                                                                {{getNumberTranslate($value_data->gift_discount_amount)}} %
                                                            @else
                                                                {{single_price($value_data->gift_discount_amount)}}
                                                            @endif
                                                        </span>
                                                    </div>
                                                </div>
                            ...
                            <div class="product_info">
                                ...
                                @php
                                    ...
                                    $showData = [
                                            'name' => $card->name,
                                            'url' => route('frontend.gift-card.show.multiple',$card->sku),
                                ...
                                @foreach ($card->addGiftCard as $m => $value_name)
                                    @if($m == 0)
                                        @php
                                            if($value_name->hasDiscount()){
                                                $base_price = selling_price($value_name->gift_selling_price, $value_name->gift_discount_type, $value_name->gift_discount_amount);
                                            }else{
                                                $base_price = $value_name->gift_selling_price;
                                            }
                                            $showData = [
                                                'name' => $card->name,
                                                'url' => route('frontend.gift-card.show.multiple',$card->sku),
                                    ...
                                    @if(isGuestAddtoCart())
                                        <div class="color_List d-flex gap_5 flex-wrap">
                                            @foreach ($card->addGiftCard as $m => $value_name)
                                                    ...
                                                    @if($value_name->gift_discount_type == 0)
                                                        <a class="attr_val_name size_btn not_111 multipleGiftCards @if ($m === 0) selected_btn @endif" color="not"
                                                            data-single-price="{{getGiftcardMultiwithDiscountPrice($value_name)}}"
                                                            data-discount-percent="{{getNumberTranslate($value_name->gift_discount_amount)}}"
                                                            data-selling-price="{{$value_name->gift_selling_price}}"
                                                            data-discount-amount="{{$value_name->gift_discount_amount}}"
                                                            data-discount-type="{{$value_name->gift_discount_type}}"
                                                            data-discount-avail="{{$value_name->hasDiscount()}}"
                                                            data-main-amount="{{getGiftcardMultiwithDiscountPrice($value_name)}}">
                                                            {{getGiftcardMultiwithDiscountPrice($value_name)}}
                                                        </a>
                                                    @else
                                                        <a class="attr_val_name size_btn not_111 multipleGiftCards @if ($m === 0) selected_btn @endif" color="not"
                                                            data-single-price ="{{getGiftcardMultiwithDiscountPrice($value_name)}}"
                                                            data-discount-amount="{{single_price($value_name->gift_discount_amount)}}"
                                                            data-selling-price="{{$value_name->gift_selling_price}}"
                                                            data-discount-amount="{{$value_name->gift_discount_amount}}"
                                                            data-discount-type="{{$value_name->gift_discount_type}}"
                                                            data-discount-avail="{{$value_name->hasDiscount()}}"
                                ...
                                @if(isGuestAddtoCart() == true)
                                    <div class="row mt_30 ">
                                        <div class="col-md-6">
                                                <a data-gift-card-type="multiple" data-gift-card-id="{{ $card->id }}" data-seller="1" data-base-price="{{$base_price}}" data-shipping-method="1" data-show="{{json_encode($showData)}}" class="cursor_pointer add_gift_card_to_cart amaz_primary_btn style2 mb_20  add_to_cart text-uppercase flex-fill text-center w-100 dataBasePrice">{{__('defaultTheme.add_to_cart')}}</a>
                                        </div>
                                        <div class="col-md-6">
                                            <a id="butItNow" class="cursor_pointer amaz_primary_btn3 mb_20  w-100 text-center justify-content-center text-uppercase buy_now_btn" data-gift-card-type="multiple"  data-gift-card-id="{{ $card->id }}" data-seller="1" data-base-price="{{$base_price}}" data-shipping-method="1" data-type="gift_card">{{__('common.buy_now')}}</a>
                                        </div>
                                    </div>

                                    <div class="add_wish_compare d-flex align-items-center mb_20">
                                ...
                                @else
                                    <div class="row mt_30 ">
                                        <div class="col-md-12">
                                            <a class="cursor_pointer  amaz_primary_btn style2 mb_20   text-uppercase flex-fill text-center w-100">{{__('defaultTheme.login_to_order')}}</a>
                                        </div>
                                    </div>
                                @endif
                            </div>
        ...
        (function($){
            ...
            $(document).ready(function(){
                ...
                $(document).on('click', ".add_gift_card_to_cart", function(event) {
                    ...
                    addToCart($(this).attr('data-gift-card-id'),$(this).attr('data-seller'),$('#qty').val(),$(this).attr('data-base-price'),$(this).attr('data-shipping-method'),'gift_card',$(this).data('show'), $(this).data('gift-card-type'))
                });
                ...
                $(document).on('click', '.multipleGiftCards', function(event){
                    event.preventDefault();
                    $(this).parent().find('.multipleGiftCards').removeClass('selected_btn');
                    $(this).addClass('selected_btn');
                    var sellingPrice = $(this).data('selling-price');
                    var discountPrice = $(this).data('discount-amount');
                    var discountType = $(this).data('discount-type');
                    var mainAmount = $(this).data('main-amount');
                    var discountPercent = $(this).data('discount-percent');
                    var discountAmount = $(this).data('discount-amount');
                    var singlePrice = $(this).data('single-price');
                    var discountAvail = $(this).data('discount-avail');
                    var htmlData = '';
                    ...
                    if(discountAvail){
                        if(discountType == 0){
                            htmlData = `
                                <div class="pro_details_disPrise d-flex align-items-center gap_15">
                                    <h4 class="discount_prise  m-0  "> <span class="text-decoration-line-through">${currency_format(sellingPrice)}</span> </h4>
                                    <span class="diccount_percents">
                                        ${discountPercent}%
                                    </span>
                                </div>
                    ...
                    let qty = $('#qty').val();
                    var dataNum = singlePrice.toString();

                    $('#unit_price').val(dataNum.slice(1, 15));

                    $('#total_price').text(currency_format(parseFloat(dataNum.slice(2, 15))*qty));
                    $('.add_gift_card_to_cart').attr('data-base-price', dataNum.slice(2, 15));
                    $('.buy_now_btn').attr('data-base-price', dataNum.slice(2, 15));
                    $('.showAmountOfGiftCard').text(mainAmount);
                    $('.showDiscountGiftCard').html(htmlData);
                });
                ...
                $(document).on("click", ".buy_now_btn", function(event){
                    event.preventDefault();
                    buyNow($(this).attr('data-gift-card-id'),$(this).attr('data-seller'),$('#qty').val(),$(this).attr('data-base-price'),$(this).attr('data-shipping-method'),'gift_card', null, $(this).data('gift-card-type'));
                });
