...
@if(isset($is_pending_payment) && $is_pending_payment)
<div class="alert alert-warning text-center" style="margin: 20px 0;">
    <h4><i class="fas fa-clock"></i> {{ __('payment_gatways.payment_pending') }}</h4>
    <p>{{ __('payment_gatways.payment_pending_description') }}</p>
    <div class="mt-3">
        <a href="{{ url('/checkout') }}" class="btn btn-outline-warning">{{ __('common.back') }} {{ __('defaultTheme.proceed_to_checkout') }}</a>
        <button onclick="location.reload()" class="btn btn-primary ml-2">{{ __('payment_gatways.check_payment_status') }}</button>
    </div>
</div>
@endif
...
<div class="container pb-5 pt-3 bg-white">
    <div class="row payments">
        <div class="col-12 col-xl-8 d-flex justify-content-end">
            <div class="checkout_v3_inner p-3">
                <div class="shiping_address_box checkout_form m-0 cart-shadown rounded-20 p-3">
                        ...
                        <div class="row">
                                ...
                                <div class="shipingV3_info mb_30">
                                    <div class="single_shipingV3_info d-flex align-items-start">
                                        ...
                                        <h5 class="m-0 flex-fill">
                                            ...
                                            @else
                                            {{!empty($address) && !empty($address->email) ? $address->email:''}}
                                            @endif</h5>
                                                ...
                                                <span class="accordion-button shadow-none" data-bs-toggle="collapse" data-bs-target="#collapse{{$key}}"  aria-controls="collapse{{$key}}">
                                                    <span class="w-100">
                                                        <label class="primary_checkbox d-inline-flex style4 gap_10 w-100" >
                                                            <input type="radio" name="payment_method" class="payment_method" data-name="{{$payment->slug}}" data-id="{{encrypt($payment->id)}}" value="{{$payment->id}}" {{$key == 0?'checked':''}}>
                                                            ...
                                                            <span class="label_name f_w_500 ">
                                                                ...
                                                                switch ($payment->slug) {
                                                                    ...
                                                                    echo '<span style="position: absolute; right:0"><img alt="" height="20" src="'.asset('public/'.$payment->logo).'"></span>';

                                                                    break;
                                                                    case 'ePayco':
                                                                    echo __("payment_gatways.epayco");
                                                                    break;
                                                                    case 'ccavenue':
                                            ...
                                            <div id="collapse{{$key}}" class="accordion-collapse collapse {{$key == 0?'show':''}}" aria-labelledby="heading{{$key}}" data-bs-parent="#accordionExample">
                                                <div class="accordion-body" id="acc_{{$payment->id}}">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        @if($payment->slug == 'cash-on-delivery')

                                                        @elseif($payment->slug == 'wallet')
                                                        ...
                                                        @elseif($payment->slug == 'google-pay')
                                                            ...
                                                            @endpush
                                                        @elseif($payment->slug == 'flutterwave')
                                                            @include('frontend.amazy.partials.payments.flutter_payment')
                                                        @elseif($payment->slug == 'bank-payment')
                                                            @include('frontend.amazy.partials.payments.bank_payment')
                                                        @elseif(isModuleActive('Bkash') && $payment->slug=="bkash")
                                                            @include('bkash::partials._checkout')
                                                        @elseif(isModuleActive('MercadoPago') && $payment->slug=="mercado-pago")
                                                            @include('mercadopago::partials._checkout_amazy')
                                                        @elseif(isModuleActive('Tabby') && $payment->slug=="tabby")
                                                            @php
                                                            ...
                                                            @endphp
                                                            @include('tabby::partials._tabby_payment_amazy')
                                                        @elseif(isModuleActive('CCAvenue') && $payment->slug=="ccavenue")
                                                            @include('ccavenue::partials._ccavenue_payment_amazy')
                                                        @elseif(isModuleActive('SslCommerz') && $payment->slug=="sslcommerce")
                                                            @include('sslcommerz::partials._checkout')
                                                        @elseif(isModuleActive('Clickpay') && $payment->slug == 'clickpay')
                                                            @include('clickpay::cart_checkout')
                                                        @elseif($payment->slug == 'ePayco')
                                                            @include('frontend.default.partials.payments.payment_epayco')
                                                        @endif
                                            ...
                                            <div class="accordion-body">
                                                ...
                                                <div class="row">
                                                    @if(auth()->check())
                                                        <div class="col-lg-12">
                                                            <div class="form-group">
                                                                ...
                                                                <select class="theme_select style2 wide radius_50px mb_20" name="address_id" id="address_id">
                                                                <option value="0">{{__('defaultTheme.new_address')}}</option>
                                                                    @foreach (auth()->user()->customerAddresses->where('is_shipping_default',0)->where('is_updated',0) as $addresss)
                                                    ...
                                                    <div class="col-lg-6 mb_20">
                                                        ...
                                                        <input class="primary_input3 style5 radius_50px" id="name" name="name" value="{{isset($billing_address)?$billing_address->name:''}}" type="text"  placeholder="{{__('common.name')}}">
                                                    ...
                                                    <div class="col-lg-6 mb_20">
                                                        ...
                                                        <input class="primary_input3 style5 radius_50px" type="text" id="address" name="address"
                                                    ...
                                                    <div class="col-lg-6 mb_20">
                                                        ...
                                                        <input class="primary_input3 style5 radius_50px" type="email" name="email" id="email" placeholder="{{__('common.email')}}" value="{{isset($billing_address)?$billing_address->email:''}}">
                                                    ...
                                                    <div class="col-lg-6 mb_20">
                                                        ...
                                                        <input class="primary_input3 style5 radius_50px" type="text" name="phone" value="{{isset($billing_address)?$billing_address->phone:''}}" id="phone" placeholder="{{__('common.phone')}}">
                                                        <span class="text-danger" id="error_phone">{{ $errors->first('phone') }}</span>
                                                    </div>
                                                    <div class="col-lg-6 mb_20">
                                                        <label class="primary_label2 style3" for="country">{{__('common.country')}} <span>*</span></label>
                                                        <select class="theme_select style2 wide radius_50px" name="country" id="country" autocomplete="off">
                                                    ...
                                                    <div class="col-lg-6 mb_20">
                                                        <label class="primary_label2 style3" for="state">{{__('common.state')}} <span>*</span></label>
                                                        <select class="theme_select style2 wide radius_50px" name="state" id="state" autocomplete="off">
                                                            <option value="">{{__('defaultTheme.select_from_options')}}</option>
                                                    ...
                                                    <div class="col-lg-6 mb_20">
                                                        <label class="primary_label2 style3" for="city">{{__('common.city')}} <span>*</span></label>
                                                        <select class="theme_select style2 wide radius_50px" name="city" id="city" autocomplete="off">
                                                            <option value="">{{__('defaultTheme.select_from_options')}}</option>
                                                    ...
                                                    <div class="col-lg-6 mb_20">
                                                        ...
                                                        <input class="primary_input3 style5 radius_50px" type="text" id="postal_code" name="postal_code" placeholder="{{__('common.postal_code')}}" value="{{isset($billing_address)?$billing_address->postal_code:''}}">
                            ...
                            <div class="col-12">
                                <div class="check_v3_btns flex-wrap d-flex align-items-center justify-content-center justify-content-md-start">
                                    @if(isModuleActive('MultiVendor'))
                                        <a href="{{url()->previous()}}" class="amaz_outline_btn">{{__('defaultTheme.return_to_information')}}</a>
                                    @else
                                        @if(!$delivery_info || $delivery_info && $delivery_info['delivery_type'] == 'home_delivery')
                                            <a href="{{url()->previous()}}" class="amaz_outline_btn">{{__('defaultTheme.return_to_shipping')}}</a>
                                        @else
                                            <a href="{{url()->previous()}}" class="amaz_outline_btn">{{__('defaultTheme.return_to_information')}}</a>
                                        @endif
                                    @endif
                                        ...
                                        @php
                                            ...
                                            if(count($gateway_activations) > 0 && $gateway_activations[0]->id == 1 || count($gateway_activations) > 0 && $gateway_activations[0]->id == 2){
                                                ...
                                                if($gateway_activations[0]->id == 1){
                                                    ...
                                                    $pay_now_btn = '<a href="javascript:void(0)" data-url="'.$url.'" data-type="CashOnDelivery" id="payment_btn_trigger" class="amaz_primary_btn style3 px-4  min_200 text-center text-uppercase">'.__("hr.order_now").'</a>';
                                                }elseif($gateway_activations[0]->id == 2){
                                                    $url = url('/checkout?').'gateway_id='.$gateway_id.'&payment_id='.$payment_id.'&step=complete_order';
                                                    $pay_now_btn = '<a href="javascript:void(0)" data-url="'.$url.'" data-type="Wallet" id="payment_btn_trigger" class="amaz_primary_btn style3 px-4  min_200 text-center text-uppercase">'.__("hr.order_now").'</a>';
                                                }
                                            }else {
                                                $method = '';
                                                ...
                                                $pay_now_btn = '<a href="javascript:void(0)" id="payment_btn_trigger" data-type="'.$method.'" class="amaz_primary_btn style3 px-4  min_200 text-center text-uppercase">'.__("hr.order_now").'</a>';
                                            }
                                        @endphp
                                        {!! $pay_now_btn !!}
                                    </div>
            ...
            <div class="p-3 flex-fill">
                @php
                    ...
                    if (isModuleActive('MultiVendor')) {
                        foreach ($cartData as $packages) {
                            foreach ($packages as $cartItem) {
                                $total_points += (float) ($cartItem->total_club_point ?? 0);
                            }
                        }
                    }
                @endphp
                            ...
                            <div class="singleVendor_product_lists">
                                <div class="singleVendor_product_list d-flex align-items-center">
                                    ...
                                    <div class="product_list_content">
                                        ...
                                        @endif
                                        <h5 class="d-flex align-items-center"><span class="product_count_text">{{$cart->qty}}<span>x</span></span>{{single_price($cart->price)}} | {{ getNumberTranslate($cart->total_club_point) }}</h5>
                                    </div>
                                </div>
                            </div>
                            @php
                ...
                <div class="subtotal_lists">
                    ...
                    <div id='tobbyFeeDiv' class="single_total_list d-flex align-items-center flex-wrap d-none">
                        ...
                        <div class="single_total_right">
                            <span id='tabby_fee' data-currency='{{ getCurrency() }}' data-tabby-fee='{{ $tabby_fee }}' data-product-total='{{ $product_price }}' data-total-amount='{{$total}}'>  </span>
                        </div>
                    </div>
                    ...
                    <div class="total_amount d-flex align-items-center flex-wrap">
                        <div class="single_total_left flex-fill">
                            <span class="total_text">{{__('common.total')}} ({{__('common.incl')}}. {{__('common.vat/tax/gst')}})</span>
                        </div>
                        <div class="single_total_right">
                            <span class="total_text" id="total_amount" data-amount='{{ str_replace(',','',number_format(convertCurrency($total),2)) }}'>{{single_price($total)}}</span>
                        </div>
                    </div>
                </div>
            </div>
