                                    ...
                                    @foreach(@$menu->menus as $key => $menu)
                                        @if(@$menu->menu->menu_type == 'mega_menu' && @$menu->menu->status == 1)
                                            <li><a class="dropdown-item has_arrow d-flex align-items-center">
                                                @php $icon = @$menu->menu->icon; @endphp
                                                @if($icon)
                                                    @if(str_contains($icon, ' ') || str_starts_with($icon, 'fa') || str_starts_with($icon, 'ti-'))
                                                        <i class="{{ $icon }}" style="font-size:16px;"></i>
                                                    @else
                                                        <x-dynamic-component :component="'lucide-' . $icon" style="width:16px;height:16px;display:inline-block;vertical-align:middle;"/>
                                                    @endif
                                                @endif
                                                ...
                                                <ul class="mega_width_menu">
                                                    @if(@$menu->menu->columns->count())
                                                        ...
                                                        @foreach(@$menu->menu->columns as $key => $column)
                                                            <li class="pt-0
                                                                ...
                                                                @endif
                                                            ">
                                                                <a class="mega_metu_title">{{ textLimit($column->column, 25) }}</a>
                                                                <ul>
                                                                    @foreach(@$column->elements as $key => $element)
                                                                        @if($element->type == 'link')
                                                                        <li><a {{$element->is_newtab == 1? 'target="_blank"':''}} href="@if($element->link != null)
                                                                            {{url($element->link)}}
                                                                            @else
                                                                            javascript:void(0);
                                                                        ...
                                                                        @elseif($element->type == 'category' && $element->category->status == 1)
                                                                            <li><a {{$element->is_newtab == 1? 'target="_blank"':''}} href="{{route('frontend.category-product',['slug' => @$element->category->slug, 'item' =>'category'])}}">{{ ucfirst(textLimit($element->title, 25)) }}</a></li>

                                                                        @elseif(@$element->type == 'product' && @$element->product)
                                                                        <li><a {{$element->is_newtab == 1? 'target="_blank"':''}} href="{{singleProductURL(@$element->product->seller->slug, $element->product->slug)}}">{{ ucfirst(textLimit($element->title,25)) }}</a></li>
                                                                         @elseif(@$element->type == 'brand' && @$element->brand->status == 1)
                                                                        <li><a {{$element->is_newtab == 1? 'target="_blank"':''}} href="{{route('frontend.category-product',['slug' => @$element->brand->slug, 'item' =>'brand'])}}">{{ ucfirst(textLimit($element->title, 25)) }}</a></li>

                                                                        @elseif($element->type == 'page' && $element->page->status == 1)
                                                                                @if(!isModuleActive('Lead') && $element->page->module == 'Lead')
                                                                                    @continue
                                                                                @endif
                                                                        <li><a {{$element->is_newtab == 1? 'target="_blank"':''}} href="{{ url(@$element->page->slug) }}">{{ ucfirst(textLimit($element->title, 25)) }}</a></li>

                                                                        @elseif($element->type == 'tag')
                                                                        <li><a {{$element->is_newtab == 1? 'target="_blank"':''}} href="{{route('frontend.category-product',['slug' => @$element->tag->name, 'item' =>'tag'])}}">{{ ucfirst(textLimit($element->title,25)) }}</a></li>
                                                    ...
                                                    @endif
                                                    <!-- single_menu  -->
                                                    <li class="d-flex flex-column-reverse flex-row-lg-reverse w-100 justify-content-between p-0 m-0 {{@$menu->menu->menuAds->status?'':'d-none'}}" style="flex-basis: 100%">
                                                        <div class="sub_menu_bg_img">
