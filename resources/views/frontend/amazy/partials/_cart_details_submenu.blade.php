@php
    ...
    $points = isset($carts) ? (float) $carts->sum('total_club_point') : 0;
@endphp
...
@else
    ...
    @if($items > 0)
        <div class="side_chartView_total d-flex align-items-center add_to_cart  gj-cursor-pointer ">
            ...
            <div class="icon_lock dynamic_svg position-relative">
                ...
                <span class="side-cart-count">{{getNumberTranslate($items)}}</span>
            </div>
            <div class="cart_view_text">
                <h5 class="lh-1 ">{{ getNumberTranslate($points) }} <span class=""> {{__('common.points')}}</span></h5>
    ...
    <div class="shoping_wrapper {{$items < 1?'d-none':''}}">
        <div class="shoping_cart">
                            ...
                            <div class="cart_content flex-fill">
                                ...
                                <div class="cart_content_text d-flex align-items-center gap_10 flex-fill flex-wrap">
                                    <div class="product_number_count style_2" data-target="amountc-1">
                                        ...
                                        <input id="sidebar_cart_qty_{{$cart->id}}" class="count_single_item input-number" type="text" data-value="{{$cart->qty}}" value="{{$cart->qty}}" readonly>
                                    ...
                                    <p><span class="prise mx-1" >{{single_price($cart->total_price)}} </span> | <span class="points mx-1" >{{ $cart->total_club_point }}</span></p>
                                </div>
                            </div>
                            @if($just_path != '/checkout')
                                <div class="cart_trash_icon d-flex align-items-center  justify-content-end cursor_pointer" id="submenu_cart_btn_{{$cart->id}}">
                                    <span class="remove_from_submenu_btn" data-id="{{$cart->id}}" data-product_id="{{$cart->product_id}}" data-btn="#submenu_cart_btn_{{$cart->id}}">
                                        <svg  width="12.249" height="15.076" viewBox="0 0 12.249 15.076">
                                            <g id="trash" transform="translate(-48)">
                                              ...
                                              <path data-name="Path 1450" d="M240.471,215.067a.471.471,0,0,0,.471-.471v-6.125a.471.471,0,1,0-.942,0V214.6A.471.471,0,0,0,240.471,215.067Z" transform="translate(-186.347 -201.875)" fill="#777"/>
                                              <path data-name="Path 1451" d="M320.471,215.067a.471.471,0,0,0,.471-.471v-6.125a.471.471,0,1,0-.942,0V214.6A.471.471,0,0,0,320.471,215.067Z" transform="translate(-263.991 -201.875)" fill="#777"/>
                                              <path data-name="Path 1452" d="M160.471,215.067a.471.471,0,0,0,.471-.471v-6.125a.471.471,0,0,0-.942,0V214.6A.471.471,0,0,0,160.471,215.067Z" transform="translate(-108.702 -201.875)" fill="#777"/>
                                            </g>
                                          </svg>
                                    </span>
                                </div>
            ...
            <div class="shoping_cart_subtotal d-flex justify-content-between align-items-center">
                <h4 class="m-0">{{__('common.totals_points')}}</h4>
