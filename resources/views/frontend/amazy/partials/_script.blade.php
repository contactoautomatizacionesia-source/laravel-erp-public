...
<script src="{{ asset(asset_path('frontend/amazy/js/patch-nice-select.js')) }}"></script>
<script>
        ...
        $(document).ready(function(){
                        ...
                        if(Object.keys(search_item).length > 0){
                            $('#search_history').html(`
                                <div class='search_product_info search_history'>
                                    <p>{{ __('common.search_history') }}</p>
                                    <strong id='clear_search'>{{__('common.clear')}}</strong>
                                    </div>
                    ...
                    if(localStorage.getItem('search_history') != null && localStorage.getItem('search_history') != undefined){
                        ...
                        if(Object.keys(search_item).length > 0){
                            $('#search_history2').html(`
                                <div class='search_product_info search_history'>
                                    <p>{{ __('common.search_history') }}</p>
                                    <strong id='clear_search2'>{{__('common.clear')}}</strong>
                                    </div>
            ...
            function resolveLocalizedText(value) {
                ...
                if (typeof value === 'object') {
                    ...
                    const firstAvailable = Object.values(value).find((item) => typeof item === 'string' && item.length > 0);
                ...
                .done(function (response) {
                    ...
                    if(response.categories.length > 0){
                        ...
                        response.categories.forEach((item) => {
                            ...
                            const categoryName = resolveLocalizedText(item.name);
                            categoryData += '<a class="store_link" data-type="category" href="'+page_url+'">'+categoryName+'</a>';
                        });
                    }
                    if(response.products.length > 0){
                        $('#product_search').html(`
                            <div class="tag_box">
                                    <a href="javascript:void(0)" class="search_title">{{__('common.products')}}</a>
                            </div>
                                    ...
                                    <div class="product_info d-flex align-items-center flex-fill gap_10">
                                        <div class="thumb">
                                            @if(app('general_setting')->lazyload == 1)
                                                <img data-src="${item.thumb_img}" alt="${item.product_name}" title="${item.product_name}" src="{{showImage(themeDefaultImg())}}" class="lazyload">
                                            @else
                    ...
                    if(response.sellers != undefined){
                        if(response.sellers.length > 0){
                            $('#seller_search').html(`
                                <div class="tag_box">
                                    <a href="#" class="search_title">{{__('common.shops')}}</a>
                                </div>
                                ...
                                sellerData += `
                                    <a href="${url}" data-type="product" class="store_link product_search_single d-flex align-items-center gap_10">
                                        <div class="product_info flex-fill d-flex gap_10 mb_10 align-items-center">
                                            <div class="thumb">
                                                <img data-src="${item.avater}" alt="${item.first_name}" title="${item.first_name}" src="{{showImage(themeDefaultImg())}}" class=lazyload>
                                            </div>
                                            <div class="product_info_text">
