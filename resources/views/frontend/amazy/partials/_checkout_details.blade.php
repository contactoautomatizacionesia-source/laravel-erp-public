...
<form class="checkout-public" action="{{route('frontend.checkout')}}" method="GET" enctype="multipart/form-data" id="mainOrderForm">
    <div class="container pt-3 pb-5">
        <div class="row">
            <div class="col-12 col-xl-8 d-flex justify-content-end">
                <div class="checkout_v3_inner">
                    @guest
                        <div class="checout_head bg-white mb-2 cart-shadown rounded-20 p-3 d-flex align-items-center gap-2">
                            <i class="ti-folder"></i>
                    ...
                    @if(isModuleActive('MultiVendor'))
                        ...
                        @foreach($cartData as $seller_id => $packages)
                                                ...
                                                @foreach($packages as $key => $item)
                                                    ...
                                                    @if($item->product_type == 'product')
                                                        @php
                                                            ...
                                                            $total_points += (float) ($item->total_club_point ?? 0);
                                                            $pro_price = 0;
                                                    ...
                                                    @if(!empty($isIntselected) && isModuleActive('INTShipping') && !empty($shipping_address))
                                                        <tr class="custom-tr" >
                                                            ...
                                                            <select name="intshipping_cartItem[]" id="uniqueCartId{{$item->id}}" class="theme_select radius_50px w-100 ck-select intshiping border-top-0" data-id="{{$item->id}}">
                                                                ...
                                                                @foreach ($rates as $rate)
                                                                    ...
                                                                    @if ($rate->base_on_item == 1)
                                                                        @if ($rate->minimum * 1000 <= $item->product->sku->weight && $rate->maximum * 1000 >= $item->product->sku->weight)
                                                                            ...
                                                                            @endphp
                                                                            <option value="{{($product_shipping_cost)}} {{$rate->id}}">{{$rate->rate_name}} - {{single_price($product_shipping_cost)}} - {{$rate->shipment_time_label}}</option>
                                                                        @endif
                            ...
                            @php
                                ...
                                $seller_shippings = $shipping_methods->where('request_by_user',$seller->id)->where('minimum_shopping','<=', $total_check);
                                if(count($seller_shippings) < 1){
                                    $seller_shippings = $shipping_methods->where('request_by_user',$seller->id)->whereIn('carrier_id',$carriers);
                                }
                            @endphp
                            @if(!isModuleActive('INTShipping'))
                        ...
                        @endforeach
                    @endif
                    <div class="shiping_address_box checkout_form m-0 cart-shadown rounded-20 p-3 bg-white">
                        ...
                        @else
                            <div class="mb_20">
                                ...
                                <input class="primary_input3 style5 radius_50px" type="email" id="email" placeholder="{{__('common.email')}}"  value="{{$shipping_address?$shipping_address->email:''}}" name="email">
                        ...
                        <div class="billing_address">
                            <div class="shipping_delivery_div justify-content-between">
                                @php
                                    $delivery_info = null;
                                @endphp
                                        ...
                                        @else
                                            @if(count($pickup_locations) > 0)
                                                ...
                                                <div class="pick_location_list_div mt_30 @if(!$delivery_info || $delivery_info && $delivery_info['delivery_type'] == 'home_delivery') d-none @endif">
                                                    <label class="primary_label2 style2" for="pickup_location">{{__('shipping.pickup_location')}} <span>*</span></label>
                                                    <select class="theme_select radius_50px style2 wide" name="pickup_location" id="pickup_location" autocomplete="off">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table-borderless">
                                            ...
                                            <tr>
                                                ...
                                                <td> {{ getNumberTranslate($shipping_address?$shipping_address->phone:$user_phone)}}</td>
                                            </tr>
                                            <tr>
                                                <td> {{__('common.postal_code_or_pin_code')}}</td>
                                                <td> {{getNumberTranslate($shipping_address?$shipping_address->postal_code:'')}}</td>
                                            </tr>
                                        </table>
                                    </div>
                                </div>
                            ...
                            <div class="row shipping_address_edit_div {{$shipping_address?'d-none':""}}">
                                ...
                                @if(auth()->check())
                                    <div class="col-lg-12">
                                        ...
                                        <select class="theme_select radius_50px style2 wide mb_20" name="address_id" id="address_id">
                                            <option value="0">{{__('defaultTheme.new_address')}}</option>
                                            @foreach (auth()->user()->customerAddresses->where('is_updated',0) as $address)
                                ...
                                <div class="col-lg-6 mb_20">
                                    <label class="primary_label2 style3" for="name">{{__('common.name')}} <span>*</span></label>
                                    <input class="primary_input3 style5 radius_50px" id="name" name="name" value="{{$shipping_address?$shipping_address->name:$user_name}}" type="text"  placeholder="{{__('common.name')}}">
                                    <span class="text-danger" id="error_name">{{ $errors->first('name') }}</span>
                                </div>

                                @if($checkoutField[0]->field_name=='address' && $checkoutField[0]->visibility==1)
                                <div class="col-lg-6 mb_20">
                                    <label class="primary_label2 style3" for="address">{{__('common.address')}} @if($checkoutField[0]->required==1) <span>*</span> @endif </label>
                                    <input class="primary_input3 style5 radius_50px" name="address" id="address" type="text"  placeholder="{{__('common.address')}}" value="{{$shipping_address?$shipping_address->address:''}}">
                                ...
                                <div class="col-lg-6 mb_20">
                                    ...
                                    <input class="primary_input3 style5 radius_50px" type="email" name="email" id="email" placeholder="{{__('common.email')}}" value="{{$shipping_address?$shipping_address->email:$user_email}}">
                                ...
                                <div class="col-lg-6 mb_20">
                                    ...
                                    <input class="primary_input3 style5 radius_50px" type="text" name="phone" data-min="{{  app('general_setting')->min_digit }}" data-max="{{  app('general_setting')->max_digit }}" value="{{$shipping_address?$shipping_address->phone:$user_phone}}" id="phone" placeholder="{{__('common.phone')}}">
                                    <span class="text-danger" id="error_phone">{{ $errors->first('phone') }}</span>
                                </div>

                                @if($checkoutField[3]->field_name=='country' && $checkoutField[3]->visibility==1)
                                <div class="col-lg-6 mb_20">
                                    <label class="primary_label2 style3" for="country">{{__('common.country')}} @if($checkoutField[3]->required==1) <span>*</span> @endif </label>
                                    <select class="theme_select radius_50px style2 wide" name="country" id="country" autocomplete="off">
                                        <option value="">{{__('defaultTheme.select_from_options')}}</option>
                                ...
                                @if($checkoutField[2]->field_name=='state' && $checkoutField[2]->visibility==1)
                                <div class="col-lg-6 mb_20">
                                    <label class="primary_label2 style3" for="state">{{__('common.state')}} @if($checkoutField[2]->required==1) <span>*</span> @endif </label>
                                    <select class="theme_select radius_50px style2 wide" name="state" id="state" autocomplete="off">
                                        <option value="">{{__('defaultTheme.select_from_options')}}</option>
                                ...
                                @if($checkoutField[1]->field_name=='city' && $checkoutField[1]->visibility==1)
                                <div class="col-lg-6 mb_20">
                                    <label class="primary_label2 style3" for="city">{{__('common.city')}} @if($checkoutField[1]->required==1)<span>*</span> @endif </label>
                                    <select class="theme_select radius_50px style2 wide" name="city" id="city" autocomplete="off">
                                        <option value="">{{__('defaultTheme.select_from_options')}}</option>
                                ...
                                @if($checkoutField[4]->field_name=='postal' && $checkoutField[4]->visibility==1)
                                <div class="col-lg-6 mb_20">
                                    <label class="primary_label2 style3" for="postal_code">{{__('common.postal_code_or_pin_code')}} @if($postalCodeRequired) @if($checkoutField[4]->required==1)<span>*</span> @endif @endif</label>
                                    <input class="primary_input3 style5 radius_50px" type="text"  id="postal_code" name="postal_code" placeholder="{{__('common.postal_code')}}" value="{{$shipping_address?$shipping_address->postal_code:''}}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12">
                                    <div class="check_v3_btns flex-wrap d-flex align-items-center justify-content-center justify-content-md-start">
                                        <a href="{{url('/cart')}}" class="amaz_outline_btn">{{__('defaultTheme.return_to_cart')}}</a>
                                        @if(isModuleActive('MultiVendor'))
                                            <input type="hidden" name="step" value="select_payment">
                                            @if(env('NOCAPTCHA_FOR_CHECKOUT') == "true" && env('NOCAPTCHA_INVISIBLE') == "true")
                                                <button type="button" class="g-recaptcha amaz_primary_btn style3 px-4  min_200 text-center text-uppercase" data-sitekey="{{env('NOCAPTCHA_SITEKEY')}}" data-size="invisible" data-callback="onSubmit">{{__('defaultTheme.continue_to_shipping')}}</button>
                                            @else
                                                <button type="submit" class="amaz_primary_btn style3 px-4  min_200 text-center text-uppercase ">{{__('defaultTheme.continue_to_shipping')}}</button>
                                            @endif
                                        @else
                                            <div id="next_step_btn_div">
                                                ...
                                                @else
                                                    ...
                                                    @if(env('NOCAPTCHA_FOR_CHECKOUT') == "true" && env('NOCAPTCHA_INVISIBLE') == "true")
                                                        <button type="button" class="g-recaptcha amaz_primary_btn style3 px-4  min_200 text-center text-uppercase" data-sitekey="{{env('NOCAPTCHA_SITEKEY')}}" data-size="invisible" data-callback="onSubmit">{{__('defaultTheme.continue_to_payment')}}</button>
                                                    @else
                                                        <button type="submit" class="amaz_primary_btn style3 px-4  min_200 text-center text-uppercase ">{{__('defaultTheme.continue_to_payment')}}</button>
                                                    @endif
                                                @endif
                                            </div>
                                        @endif
                    ...
                    @if(!isModuleActive('MultiVendor'))
                        ...
                        @foreach($cartData as $key => $cart)
                                ...
                                <div class="singleVendor_product_lists">
                                    <div class="singleVendor_product_list d-flex align-items-center cart_thumb_div">
                                        ...
                                        <div class="product_list_content">
                                            ...
                                            @endif
                                            <h5 class="d-flex align-items-center"><span class="product_count_text">{{getNumberTranslate($cart->qty)}}<span>x</span></span>{{single_price($cart->price)}} | {{ getNumberTranslate($cart->total_club_point) }}</h5>
                                        </div>
                                    </div>
                                </div>
                                @php
                                    $actual_price += $cart->total_price;
                                    $total_points += (float) ($cart->total_club_point ?? 0);
                                    if (isModuleActive('WholeSale')){
                                        $w_main_price = 0;
                            ...
                            @else
                                ...
                                @php
                                    $actual_price += $cart->total_price;
                                    $tax += $cart->giftCard->tax * $cart->qty;
                                    $subtotal += ($cart->giftCard->sell_price * $cart->qty) + $tax;
                                @endphp
                            @endif
                        @endforeach
                    @endif
                    ...
                    <div class="subtotal_lists">
                        <div class="single_total_list d-flex align-items-center">
                            ...
                            <div class="single_total_right">
                                @if(Session::has('auction_type'))
                                    <span>+ {{single_price(Session::get('auction_price'))}}</span>
                                @else
                                    @if (app('general_setting')->price_with_vat)
                        ...
                        <div class="single_total_list d-flex align-items-center">
                            <div class="single_total_left flex-fill">
                                <h4>{{ __('common.totals_points') }}</h4>
                            </div>
                            <div class="single_total_right">
                                <span>{{ getNumberTranslate($total_points) }}</span>
                            </div>
                        </div>
