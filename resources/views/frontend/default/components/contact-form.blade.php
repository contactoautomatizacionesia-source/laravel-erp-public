                        ...
                        @foreach($form_data as $row)
                            @php
                                if($row->type != 'header' && $row->type !='paragraph'){
                                    ...
                                    $labelText = property_exists($row,'label') ? $row->label : '';
                                    $placeholderText = property_exists($row,'placeholder') ? $row->placeholder : $labelText;
                                    $label = translateWithFallback('formBuilder.'.$labelText, $labelText);
                                    $placeholder = translateWithFallback('formBuilder.'.$placeholderText, $placeholderText);
                                }
                            @endphp
                                ...
                                @elseif($row->type == 'text' || $row->type == 'number' || $row->type == 'email' || $row->type == 'date')
                                    <div class="form-group">
                                            <label for="{{$row->name}}"> {{$label}} @if($required) <span class="text-danger">*</span> @endif</label>
                                ...
                                @elseif($row->type == 'date')
                                    <div class="form-group">
                                        <label for="datepicker"> {{$label}} @if($required) <span class="text-danger">*</span> @endif</label>
