...
Route::post('/register/scan-id', [RegisterController::class, 'scanId'])->name('register.scan_id');
...
Route::post('/change-receive-status-by-customer', [OrderController::class, 'changeReceiveStatusByCustomer'])->name('change_receive_status_by_customer');
//seller profile
...
Route::group(['middleware' => ['auth', 'seller'], 'prefix' => 'media-manager'], function () {
    ...
    Route::post('/bulk-delete', [MediaManagerController::class, 'mediaBulkDelete'])->name('media-manager.bulk_delete');
});
//merchant register
...
Route::get('/gift-cards/multiple/{slug}', [GiftCardController::class, 'showMultiple'])->name('frontend.gift-card.show.multiple');
...
Route::group(['middleware' => ['auth', 'customer']], function () {
    ...
    //digitalgiftcard
    Route::get('/digital-card', [DigitalGiftCardController::class, 'index'])->name('frontend.digital_gift_card');
    Route::get('/digital-card/filter-by-type', [DigitalGiftCardController::class, 'filterByType'])->name('frontend.digital_gift_card.filter_by_type');
    Route::get('/digital-card/fetch-data', [DigitalGiftCardController::class, 'fetchData'])->name('frontend.digital_gift_card.fetch-data');
    //support ticket
...
Route::group(['middleware' => ['auth', 'admin']], function () {
    Route::middleware('permission')->prefix('hr')->group(function () {
        ...
        Route::get('/staffs-restore/{id}', [StaffController::class, 'restore'])->name('staffs.restore')->middleware('prohibited_demo_mode');
        ...
        Route::get('/staff/destroy/{id}', [StaffController::class, 'destroy'])->name('staffs.destroy.get')->middleware('prohibited_demo_mode');
    });
...
// --- Rutas para el Sistema de Doble Aprobación ---
Route::group(['middleware' => ['auth', 'admin']], function () {
    // Ruta para recuperar los datos y DataTables
    Route::get('/double-approval/review/data', [\App\Http\Controllers\DoubleApprovalController::class, 'showReviewModal'])
        ->name('double_approval.review');

    // Ruta para actualizar el estado de una solicitud
    Route::post('/double-approval/update-status', [\App\Http\Controllers\DoubleApprovalController::class, 'updateStatus'])
        ->name('double_approval.update_status');

    // Ruta para vista de datatable de solicitudes pendientes
    Route::get('/double-approval/pending-list', [\App\Http\Controllers\DoubleApprovalController::class, 'index'])
        ->name('double_approval.index')
        ->middleware('permission');
});

//for profile
Route::group(['middleware' => ['auth', 'customer', 'kyc.updated'], 'prefix' => 'profile'], function () {
    ...
    Route::post('/referral/share', [ReferralController::class, 'shareReferralCode'])->name('customer_panel.referral.share');
    Route::get('/network', [ProfileController::class, 'network'])->name('frontend.profile.network');
    ...
    // --- RUTAS DE REVALIDACIÓN KYC ---
    // Nota: Estas rutas NO deben tener el middleware 'kyc.updated' para evitar un bucle de redirección infinito.
    Route::get('/kyc-update', [ProfileController::class, 'kycUpdateIndex'])->name('kyc.update.index');
    Route::post('/kyc-update', [ProfileController::class, 'kycUpdateStore'])->name('kyc.update.store');

    // File Explorer routes moved to Modules/DigitalFolder/Routes/web.php
});
...
Route::get('/db-im', [DemoController::class, 'dbImport'])->name('dbImport');
Route::get('/jazzcash', [DemoController::class, 'testJazzCash']);
Route::get('/email', [DemoController::class, 'email'])->name('email');
